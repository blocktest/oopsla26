...
public class DatadogOutputReporter implements Reporter {
    ...
    static final AgentLogger logger = AgentLogger.getLogger(DatadogOutputReporter.class.getName());
        ...
        while (!queue.isEmpty()) {
            ...
            if (value instanceof Number) {
                ...
                ((Map) value).forEach((nestedKey, nestedValue) -> {
                    // BLOCKTEST EVAL: https://github.com/uber-common/jvm-profiler/blob/07ecfce5bbae89cae1415296d4c6eb7cdd7b7dfa/src/main/java_datadog/com/uber/profiling/reporters/DatadogOutputReporter.java#L42-L50
                    blocktest().given(key, "foo").given(nestedKey, "bar").given(individualMetrics, new ArrayList<>()).given(queue, new LinkedList<>()).given(nestedValue, new Object()).checkTrue(!queue.isEmpty());
