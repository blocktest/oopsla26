            ...
            if (StringUtils.isNotEmpty(key)) {
                ...
                if (String.class.isInstance(normalizedValue) && line.contains("${")) {
                    StringSubstitutor substitutor = stringStringSubstitutorPerSection.computeIfAbsent(
                            section.getValue(), s -> {
                                // BLOCKTEST EVAL: https://github.com/vincentrussell/java-ini-parser/blob/954dd3836aa39dd170202f95e356da1eb86c894d/src/main/java/com/github/vincentrussell/ini/Ini.java#L152-L157
                                // @blocktest().given(variables, new HashMap<>()).setup(() -> {variables.put("a", "b");}).mock("getMapForSection(section)", new HashMap<String, Object>()).given(section, new MutableObject<>(NO_SECTION)).checkTrue(lambdaReturn.toString().contains("org.apache.commons.text.StringSubstitutor"));
