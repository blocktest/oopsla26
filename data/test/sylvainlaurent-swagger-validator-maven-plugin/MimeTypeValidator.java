    ...
    private void validateMimeTypesInCollection(Collection<String> collection, String message) {
        if (collection != null) {
            collection.stream().filter(x ->
                                       {
                                           // BLOCKTEST EVAL: https://github.com/sylvainlaurent/swagger-validator-maven-plugin/blob/980ad67cf33eb53ba27d44738bb8fe3aac3dba01/src/main/java/com/github/sylvainlaurent/maven/swaggervalidator/semantic/validator/path/MimeTypeValidator.java#L73-L80
                                           blocktest().given(x, "").checkReturnTrue();
                                           blocktest().given(x, "audio/mp4").checkReturnFalse();
                                           try {
                                               System.out.println(MediaType.getMimeTypes());
