...
final class ExpressionMethodVisitor extends MethodVisitor {
    ...
    public void visitInsn(int opcode) {
        ...
        switch (opcode) {
            ...
            case Opcodes.F2L:
            case Opcodes.F2D:
                // BLOCKTEST EVAL: https://github.com/streamx-co/ExTree/blob/2cb6b78c0eb4dc25fa81f582641b5464779a93fa/src/main/java/co/streamx/fluent/extree/expression/ExpressionMethodVisitor.java#L435C1-L437C94
                /*
                @blocktest("testDouble").noInit(first).noInit(e).given(opcode,Opcodes.F2D).setup(() -> {
                    _exprStack = new ExpressionStack();
                    _exprStack.push(Expression.constant(1f, Float.TYPE));
                }).checkTrue(e != null).checkEq(e.toString(), "(double)1.0");
                */
                /*
                @blocktest("testLong").noInit(first).noInit(e).given(opcode,Opcodes.F2L).setup(() -> {
                    _exprStack = new ExpressionStack();
                    _exprStack.push(Expression.constant(1f, Float.TYPE));
                }).checkTrue(e != null).checkEq(e.toString(), "(long)1.0");
                */
                /*
                @blocktest("testInt").noInit(first).noInit(e).given(opcode,Opcodes.F2I).setup(() -> {
                    _exprStack = new ExpressionStack();
                    _exprStack.push(Expression.constant(1f, Float.TYPE));
                }).checkTrue(e != null).checkEq(e.toString(), "(int)1.0");
                */
