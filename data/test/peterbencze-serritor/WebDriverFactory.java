    ...
    private static ChromeDriver createChromeDriver(final Capabilities capabilities) {
        ...
        if (capability instanceof Map) {
            ...
            extraOptions.forEach((key, value) -> {
                // BLOCKTEST EVAL: https://github.com/peterbencze/serritor/blob/158939359c0b0608686afd34bba582c9157dd362/src/main/java/com/github/peterbencze/serritor/internal/WebDriverFactory.java#L95-L105
                // MUST PROVIDE TYPE
                blocktest().given(key, "binary").given(value, "/path", "Object").given(options, new ChromeOptions())
                        .given(extraOptions, new java.util.HashMap<String, Object>(){{put("binary", "/path");}}, "java.util.HashMap<String, Object>")
                        .checkEq(options.asMap().toString(), "{browserName=chrome, goog:chromeOptions={args=[], binary=/path, extensions=[]}}");

                blocktest().given(key, "args").given(value, new java.util.ArrayList<>(java.util.Arrays.asList("1", "2"))).given(options, new ChromeOptions())
                        .given(extraOptions, new java.util.HashMap<String, Object>())
                        .checkEq(options.asMap().toString(), "{browserName=chrome, goog:chromeOptions={args=[1, 2], extensions=[]}}");

                blocktest().given(key, "extensions").given(value, new java.util.ArrayList<>(java.util.Arrays.asList("1", "2"))).given(options, new ChromeOptions())
                        .given(extraOptions, new java.util.HashMap<String, Object>())
                        .checkEq(options.asMap().toString(), "{browserName=chrome, goog:chromeOptions={args=[], extensions=[1, 2]}}");

                blocktest().given(key, "foo").given(value, "bar").given(options, new ChromeOptions())
                        .given(extraOptions, new java.util.HashMap<String, Object>())
                        .checkEq(options.asMap().toString(), "{browserName=chrome, goog:chromeOptions={args=[], extensions=[], foo=bar}}");
