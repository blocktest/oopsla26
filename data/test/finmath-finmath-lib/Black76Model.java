...
public final class Black76Model {
			...
			boolean isCall)
	{
		...
		if (beta <= bLower)
		{
			// BLOCKTEST EVAL: https://github.com/finmath/finmath-lib/blob/ad5d22e93e335cd88c95f8f2d7ba6336e3947e68/src/main/java/net/finmath/equities/models/Black76Model.java#L349-L361
			lambdatest().args(-2.5).given(sqrtPi, Math.sqrt(2.0 * Math.PI)).given(x, -15).given(bMax, Math.exp(0.5 * 5)).given(beta, 1.5).checkReturnEq(-2.73995, 0.01);
			final Function<Double, Double> HouseholderStep = sigma2 ->
			{
				final Double[] derivatives = BlackFunctionDerivatives.apply(sigma2);
				...
				return sigma2 + nu * (1.0 + 0.5 * nu * gamma) / (1.0 + nu * (gamma + delta * nu / 6.0));
			};
		...
		{
			// BLOCKTEST EVAL: https://github.com/finmath/finmath-lib/blob/ad5d22e93e335cd88c95f8f2d7ba6336e3947e68/src/main/java/net/finmath/equities/models/Black76Model.java#L332-L343
			lambdatest().args(-2.5).given(sqrtPi, Math.sqrt(2.0 * Math.PI)).given(x, -15).given(bMax, Math.exp(0.5 * 5)).given(beta, 1.5).checkReturnEq(-2.73995, 0.01);
			final Function<Double, Double> HouseholderStep = sigma2 ->
			{
				final Double[] deriv = BlackFunctionDerivatives.apply(sigma2);
