...
import org.apache.maven.artifact.versioning.VersionRange;import org.apache.maven.monitor.logging.DefaultLog;import org.blocktest.BTest;
...
import org.codehaus.plexus.logging.console.ConsoleLogger;import org.codehaus.plexus.util.FileUtils;
    ...
    if (((type.equals(Library.SHARED) || type.equals(Library.JNI) || type.equals(Library.EXECUTABLE))) && !skipDepLink) {
      ...
      if (depLibOrder != null && !depLibOrder.isEmpty()) {
        ...
        for (final Object aDepLibOrder : depLibOrder) {
          // BLOCKTEST EVAL: https://github.com/maven-nar/nar-maven-plugin/blob/e747b5ee080b243e0b817f722a43c9c203bb4df8/src/main/java/com/github/maven_nar/NarCompileMojo.java#L352C1-L362C12
          blocktest().given(tmp, new LinkedList()).given(depLibs, new LinkedList<>(Arrays.asList(new NarArtifact(new org.apache.maven.artifact.DefaultArtifact("groupId2", "artifactId2", "1.0.0", "scope", "type", "classifier", null), new NarInfo("groupId2", "artifactId2", "1.0.0", new DefaultLog(new ConsoleLogger(1, "f")), null)), new NarArtifact(new org.apache.maven.artifact.DefaultArtifact("groupId", "artifactId", "1.0.0", "scope", "type", "classifier", null), new NarInfo("groupId", "artifactId", "1.0.0", new DefaultLog(new ConsoleLogger(1, "f")), null))))).given(aDepLibOrder, "groupId:artifactId")
                  .checkTrue(tmp.size() == 1).checkTrue(depLibs.size() == 1)
                  .checkTrue(((NarArtifact) tmp.get(0)).getGroupId().equals("groupId"))
                  .checkTrue(((NarArtifact) depLibs.get(0)).getGroupId().equals("groupId2"));

          blocktest().given(tmp, new LinkedList()).given(depLibs, new LinkedList<>(Arrays.asList(new NarArtifact(new org.apache.maven.artifact.DefaultArtifact("groupId2", "artifactId2", "1.0.0", "scope", "type", "classifier", null), new NarInfo("groupId2", "artifactId2", "1.0.0", new DefaultLog(new ConsoleLogger(1, "f")), null)), new NarArtifact(new org.apache.maven.artifact.DefaultArtifact("groupId", "artifactId", "1.0.0", "scope", "type", "classifier", null), new NarInfo("groupId", "artifactId", "1.0.0", new DefaultLog(new ConsoleLogger(1, "f")), null))))).given(aDepLibOrder, "groupId3:artifactId3")
                  .checkTrue(tmp.size() == 0).checkTrue(depLibs.size() == 2);

          blocktest().given(tmp, new LinkedList()).given(depLibs, new LinkedList<>(Arrays.asList(new NarArtifact(new org.apache.maven.artifact.DefaultArtifact("groupId", "artifactId", "1.0.0", "scope", "type", "classifier", null), new NarInfo("groupId2", "artifactId2", "1.0.0", new DefaultLog(new ConsoleLogger(1, "f")), null)), new NarArtifact(new org.apache.maven.artifact.DefaultArtifact("groupId", "artifactId", "1.0.0", "scope", "type", "classifier", null), new NarInfo("groupId", "artifactId", "1.0.0", new DefaultLog(new ConsoleLogger(1, "f")), null))))).given(aDepLibOrder, "groupId:artifactId")
                  .checkTrue(tmp.size() == 2).checkTrue(depLibs.size() == 0);
