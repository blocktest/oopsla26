...
public class ASMAccessorOptimizer extends AbstractOptimizer implements AccessorOptimizer {
  ...
  public Object blockTestDummy = null;
  ...
  private Object getMethod(Object ctx, String name)
    ...
    if (tk.length() == 0) {
      ...
      if (pCtx.isStrictTypeEnforcement()) {
        ...
        // BLOCKTEST EVAL: https://github.com/mvel/mvel/blob/236d37bc78a158553592afedbbf1aa27815efb78/src/main/java/org/mvel2/optimizers/impl/asm/ASMAccessorOptimizer.java#L1833C1-L1842C10
        blocktest().noInit(es).given(args, new Object[]{null, null, Double.valueOf("5"), new Object()}).given(argTypes, new Class[]{null, Float.class, Integer.class, Float.class}).mock("es[i].getKnownEgressType()", Object.class, Integer.class)
                .checkEq(argTypes[0], null).checkEq(argTypes[1], Float.class).checkEq(argTypes[2], Integer.class).checkEq(argTypes[3], Float.class);
        for (int i = 0; i < args.length; i++) {
    ...
    if (es != null && m != null && m.isVarArgs() && (es.length != parameterTypes.length || !(es[es.length-1] instanceof ExecutableAccessor))) {
      ...
      if("null".equals(tk)){ //if null is the token no need for wrapping
        ...
        // BLOCKTEST EVAL: https://github.com/mvel/mvel/blob/236d37bc78a158553592afedbbf1aa27815efb78/src/main/java/org/mvel2/optimizers/impl/asm/ASMAccessorOptimizer.java#L1991C1-L1996C48
        blocktest().noInit(varArgExpr).given(varargsTypeName, "String").given(varArgStart, 1).given(subtokens, Arrays.asList(new char[]{'a', 'b'}, new char[]{'c', 'd'}, new char[]{'e', 'f'}))
                .checkEq(varArgExpr, "new String[] {cd,ef}").start(FIRST_BLOCK, 2);
      }
