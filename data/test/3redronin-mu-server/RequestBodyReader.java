        ...
        try {
            ...
            if (soFar > maxSize) {
                ...
                onRequestBodyRead0(content, last, error -> {
                    // BLOCKTEST EVAL: https://github.com/3redronin/mu-server/blob/69683cf8009d7a0ac52452fd4b0abe06ffd0bf87/src/main/java/io/muserver/RequestBodyReader.java#L70-L77
                    blocktest().given(callback, new DoneCallback() {
                        @Override
                        public void onComplete(Throwable error) throws Exception {

                        }
                    }).given(error, null).given(last, false).given(future, new CompletableFuture<>()).checkFlow(IfStmt().ElseIf());
                    blocktest().given(callback, new DoneCallback() {
                        @Override
                        public void onComplete(Throwable error) throws Exception {

                        }
                    }).given(error, null).given(last, true).given(future, new CompletableFuture<>()).checkFlow(IfStmt().ElseIf().Then());
                    blocktest().given(callback, new DoneCallback() {
                        @Override
                        public void onComplete(Throwable error) throws Exception {

                        }
                    }).given(error, new FileNotFoundException()).given(last, true).given(future, new CompletableFuture<>()).checkFlow(IfStmt().Then());
                    if (error != null) {
            ...
            try {
                if (content.readableBytes() > 0) {
                    DoneCallback successCalled = !last ? callback : error -> {
                        // BLOCKTEST EVAL: https://github.com/3redronin/mu-server/blob/69683cf8009d7a0ac52452fd4b0abe06ffd0bf87/src/main/java/io/muserver/RequestBodyReader.java#L146-L153
                        blocktest().given(readListener, new RequestBodyListener() {
                            @Override
                        ...
                        }).given(callback, new DoneCallback() {
                            @Override
                            public void onComplete(Throwable error) throws Exception {

                            }
                        }).given(error, null).checkFlow(IfStmt().Then());
                        blocktest().given(readListener, new RequestBodyListener() {
                            @Override
                            public void onDataReceived(ByteBuffer buffer, DoneCallback doneCallback) throws Exception {
                        ...
                        }).given(error, new FileNotFoundException()).checkFlow(IfStmt().Else());
                        if (error == null) {
