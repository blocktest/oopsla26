                    ...
                    try {
                        ...
                        while (thrownException.get() == null) {
                            ...
                            stage.whenComplete((o, throwable) -> {
                                // BLOCKTEST EVAL: https://github.com/3redronin/mu-server/blob/69683cf8009d7a0ac52452fd4b0abe06ffd0bf87/src/test/java/io/muserver/AsyncSsePublisherTest.java#L101-L106
                                blocktest().given(throwable, null).given(thrownException, new AtomicReference<>()).given(exceptionThrownLatch, new CountDownLatch(1))
                                        .checkEq(thrownException.get(), null).checkEq(exceptionThrownLatch.getCount(), 1);
                                blocktest().given(throwable, new FileNotFoundException()).given(thrownException, new AtomicReference<>()).given(exceptionThrownLatch, new CountDownLatch(1))
                                        .checkEq(thrownException.get().toString(), "java.io.FileNotFoundException").checkEq(exceptionThrownLatch.getCount(), 0);
                                if (throwable != null) {
