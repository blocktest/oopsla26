					...
					if (img == null) {
						...
						if (images != null) {
							...
							if (!src.startsWith("http")) { // relative src references only
								// BLOCKTEST EVAL: https://github.com/albfernandez/itext2/blob/5ac684e2140a3d9a5b4c34dfd2639dcc87884ad0/src/main/java/com/lowagie/text/html/simpleparser/HTMLWorker.java#L285C1-L290C10
                                blocktest().given(img, null).given(src, "testing").given(interfaceProps, new HashMap(){{put("img_baseurl", "/foo");}})
                                    .checkEq(src, "/footesting").expect(java.io.FileNotFoundException.class);
                                blocktest().given(img, null).given(src, "testing").given(interfaceProps, new HashMap(){{put("img_baseurls", "/foo");}})
                                    .checkEq(src, "testing");
