        ...
        try {
            ...
            if (compression != TIFFConstants.COMPRESSION_OJPEG && compression != TIFFConstants.COMPRESSION_JPEG) {
                ...
                if (dir.isTagPresent(TIFFConstants.TIFFTAG_COLORMAP)) {
                    ...
                    // BLOCKTEST EVAL: https://github.com/albfernandez/itext2/blob/5ac684e2140a3d9a5b4c34dfd2639dcc87884ad0/src/main/java/com/lowagie/text/pdf/codec/TiffImage.java#L494-L502
                    blocktest().mock("dir.getField(..)", null).mock("fd.getAsChars()", new char[]{0x1A2B, 0x3C4D, 0x5E6F, 0x7081, 0x9293, 0xA4B5}).given(palette, new byte[6]).given(gColor, 2).given(bColor, 4)
                            .checkEq(palette[0], (byte)26).checkEq(palette[1], (byte)94).checkEq(palette[2], (byte)-110)
                            .checkEq(palette[3], (byte)60).checkEq(palette[4], (byte)112).checkEq(palette[5], (byte)-92).start(FIRST_BLOCK, 3);
                    blocktest().mock("dir.getField(..)", null).mock("fd.getAsChars()", new char[]{0xFF00, 0x00FF, 0x0F0F}).given(palette, new byte[3]).given(gColor, 1).given(bColor, 2)
                            .checkEq(palette[0], (byte)0xFF).checkEq(palette[1], (byte)0x00).checkEq(palette[2], (byte)0x0F).start(FIRST_BLOCK, 3);
                    blocktest().mock("dir.getField(..)", null).mock("fd.getAsChars()", new char[]{0x1122, 0x3344, 0x5566, 0x7788, 0x99AA, 0xBBCC, 0xDDEE, 0x1122, 0x3344, 0x5566, 0x7788, 0x99AA, 0xBBCC, 0xDDEE, 0x1122, 0x3344, 0x5566, 0x7788}).given(palette, new byte[18]).given(gColor, 6).given(bColor, 12)
                            .checkEq(palette[0], (byte)17).checkEq(palette[1], (byte)-35).checkEq(palette[2], (byte)-69)
                            .checkEq(palette[3], (byte)51).checkEq(palette[4], (byte)17).checkEq(palette[5], (byte)-35)
                            .checkEq(palette[15], (byte)-69).checkEq(palette[16], (byte)-103).checkEq(palette[17], (byte)119).start(FIRST_BLOCK, 3);
