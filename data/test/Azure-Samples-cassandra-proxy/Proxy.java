                    ...
                    if (!onlySource) {
                        ...
                        CompositeFuture.all(f1, f2).onComplete(e -> {
                            ...
                            if (prepareCache != null) {
                                BufferCodec.PrimitiveBuffer buffer2 = BufferCodec.createPrimitiveBuffer(buf);
                                Frame r1 = clientCodec.decode(buffer2);
                                blocktest().given(prepareMap, new ConcurrentHashMap<>()).given(r1.message, new Prepared(new byte[]{1,2,3,4}, new byte[]{5,6,7,8}, null, null), "Message")
                                        .given(prepareCache, new Prepare("foo", "bar")).checkEq(prepareMap.get(ByteBuffer.wrap(new byte[]{1,2,3,4})).toString(), new Prepare("foo", "bar").toString());
                                blocktest().given(prepareMap, new ConcurrentHashMap<>()).given(r1.message, new SchemaChangeEvent("1", "2", "3", "4", null), "Message")
                                        .given(prepareCache, new Prepare("foo", "bar")).checkEq(prepareMap.get(ByteBuffer.wrap(new byte[]{1,2,3,4})), null);
                                    ...
                                    (sourceState != FastDecode.State.error))
                            {
                                ...
                                blocktest().given(prepareMap, new ConcurrentHashMap<>()).given(state, FastDecode.State.query).given(clientBuffer, BufferImpl.buffer("aaaaaaaaaa")).mock("FastDecode.getQuery(clientBuffer)", "foo").checkFlow(IfStmt().Then());
                                blocktest().given(prepareMap, new ConcurrentHashMap<>()).given(state, FastDecode.State.execute).given(clientBuffer, BufferImpl.buffer("aaaaaaaaaa")).mock("FastDecode.getQueryId(clientBuffer)", new byte[]{1,2,3,4}).checkFlow(IfStmt());
                                blocktest().given(prepareMap, new ConcurrentHashMap<ByteBuffer, Prepare>(){{ put(ByteBuffer.wrap(new byte[]{1,2,3,4}), new Prepare("abc", "def")); }}).given(state, FastDecode.State.execute).given(clientBuffer, BufferImpl.buffer("aaaaaaaaaa")).mock("FastDecode.getQueryId(clientBuffer)", new byte[]{1,2,3,4}).checkFlow(IfStmt());
