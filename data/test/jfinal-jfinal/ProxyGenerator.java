	...
	public ProxyClass generate(Class<?> target) {
		...
		for (Method m : methodArray) {
			...
			List<String> paraTypes = Arrays.asList(paras).stream().map(
				x -> {
                    // BLOCKTEST EVAL: https://github.com/jfinal/jfinal/blob/076e76a4941c989d7f535035295aa3d055123131/src/main/java/com/jfinal/proxy/ProxyGenerator.java#L98-L111
					blocktest().given(x, Arrays.stream(Arrays.stream(com.jfinal.kit.Okv.class.getDeclaredMethods()).filter(z -> z.getName().equals("keep")).iterator().next().getParameters()).iterator().next())
					    .checkReturnEq("java.lang.String...");
                    blocktest().given(x, Arrays.stream(Arrays.stream(ProxyGenerator.class.getDeclaredMethods()).filter(z -> z.getName().equals("isSkipMethod")).iterator().next().getParameters()).iterator().next())
                        .checkReturnEq("java.lang.reflect.Method");
                    // 参考 JDK Parameter
