		...
		case "Uniform": {
			if (table.containsKey("mean") && table.containsKey("cov")) {
				...
				if (scorrel.length!=dim*dim)
					throw new ArrayIndexOutOfBoundsException("Size of a parameter vector does not match distribution dimension.");
				// BLOCKTEST EVAL: https://github.com/loehndorf/scengen/blob/5e11067f76f5631a31b75f823cd3666a769b098f/src/main/java/scengen/application/Generator.java#L186C3-L195C6
				blocktest("testA").given(dim, 3).given(scorrel, new String[]{"1", "2", "1", "1", "1", "1", "1", "1", "1"}).expect(IllegalArgumentException.class).end(FIRST_BLOCK);
				blocktest("testB").given(dim, 3).given(scorrel, new String[]{"1", "1", "2", "1", "1", "1", "1", "1", "1"}).expect(IllegalArgumentException.class).end(FIRST_BLOCK);
				blocktest("testC").given(dim, 3).given(scorrel, new String[]{"1", "1", "1", "-2", "1", "1", "1", "1", "1"}).expect(IllegalArgumentException.class).end(FIRST_BLOCK);
				blocktest("testD").given(dim, 3).given(scorrel, new String[]{"-1", "1", "1", "1", "1", "1", "1", "1", "1"}).expect(IllegalArgumentException.class).end(FIRST_BLOCK);
				blocktest("testE").given(dim, 3).given(scorrel, new String[]{"1", "1", "1", "1", "-1", "1", "1", "1", "1"}).expect(IllegalArgumentException.class).end(FIRST_BLOCK);
				blocktest("testF").given(dim, 3).given(scorrel, new String[]{"1", "1", "1", "1", "1", "1", "1", "1", "-1"}).expect(IllegalArgumentException.class).end(FIRST_BLOCK);
				// @blocktest("testG").given(dim, 3).given(scorrel, new String[]{"1", "1", "1", "1", "1", "1", "1", "-1", "1"}).checkEq(correl[2][1], -1, 0.1).end(FIRST_BLOCK);
				// @blocktest("testH").given(dim, 3).given(scorrel, new String[]{"1", "0.5", "-0.2", "-1", "1", "0.99", "-0.3", "1", "1"}).checkEq(correl[1][1], 1, 0.1).end(FIRST_BLOCK);
