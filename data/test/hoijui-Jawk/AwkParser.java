...
////				read();
		...
		if (c == '"') {
			...
			while (token != _EOF_ && c != '"' && c != '\n') {
				// BLOCKTEST EVAL: https://github.com/hoijui/Jawk/blob/33cfbc2bc14bc9c75fedccbe2963ef26b61543dc/src/main/java/org/jawk/frontend/AwkParser.java#L697C1-L710C6
				blocktest().given(c, '\\').given(string, new StringBuffer()).mock("read()", c='t').checkEq(string.toString(), "\t");
				blocktest().given(c, '\\').given(string, new StringBuffer()).mock("read()", c='n').checkEq(string.toString(), "\n");
				blocktest().given(c, '\\').given(string, new StringBuffer()).mock("read()", c='r').checkEq(string.toString(), "\r");
				blocktest().given(c, '\\').given(string, new StringBuffer()).mock("read()", c='o').checkEq(string.toString(), "o");
				blocktest().given(c, 'f').given(string, new StringBuffer()).mock("read()", c='o').checkEq(string.toString(), "f");
				if (c == '\\') {
					read();
					if (c == 't') {
						string.append('\t');
