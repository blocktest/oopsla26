...
import static org.blocktest.types.Flow.IfStmt;import static org.blocktest.utils.Constant.*;
			...
			while (!position.isEOF()) {
				...
				switch (opcode) {
					...
					case AwkTuples._POW_EQ_ARRAY_: {
						...
						// BLOCKTEST EVAL: https://github.com/hoijui/Jawk/blob/33cfbc2bc14bc9c75fedccbe2963ef26b61543dc/src/main/java/org/jawk/backend/AVM.java#L563C1-L584C8
						blocktest().given(opcode, AwkTuples._PLUS_EQ_ARRAY_).given(orig_val, 5).given(val, 2).checkEq(new_val, 7, 0.001).start(FIRST_ASSIGN);
						blocktest().given(opcode, AwkTuples._MINUS_EQ_ARRAY_).given(orig_val, 5).given(val, 2).checkEq(new_val, 3, 0.001).start(FIRST_ASSIGN);
						blocktest().given(opcode, AwkTuples._MULT_EQ_ARRAY_).given(orig_val, 5).given(val, 2).checkEq(new_val, 10, 0.001).start(FIRST_ASSIGN);
						blocktest().given(opcode, AwkTuples._DIV_EQ_ARRAY_).given(orig_val, 5).given(val, 2).checkEq(new_val, 2.5, 0.001).start(FIRST_ASSIGN);
						blocktest().given(opcode, AwkTuples._MOD_EQ_ARRAY_).given(orig_val, 5).given(val, 2).checkEq(new_val, 1, 0.001).start(FIRST_ASSIGN);
						blocktest().given(opcode, AwkTuples._POW_EQ_ARRAY_).given(orig_val, 5).given(val, 2).checkEq(new_val, 25, 0.001).start(FIRST_ASSIGN);
						blocktest().given(opcode, AwkTuples._ASSIGN_).given(orig_val, 5).given(val, 2).expect(Error.class).start(FIRST_THROW);
					...
					case AwkTuples._SRAND_: {
						...
						if (numargs == 0) {
							...
							// BLOCKTEST EVAL: https://github.com/hoijui/Jawk/blob/33cfbc2bc14bc9c75fedccbe2963ef26b61543dc/src/main/java/org/jawk/backend/AVM.java#L849-L862
							blocktest().given(o, Double.valueOf(12)).checkEq(seed, 12).start(FIRST_BLOCK);
							blocktest().given(o, Integer.valueOf(12)).checkEq(seed, 12).start(FIRST_BLOCK);
							blocktest().given(o, new String("12")).checkEq(seed, 12).start(FIRST_BLOCK);
							blocktest().given(o, new String("foo")).checkEq(seed, 0).start(FIRST_BLOCK);
						}
