    ...
    public Optional<ConnectableDTO> findByName(ProcessGroupFlowEntity componentSearch, String name){
        ...
        if (!result.isPresent())
            result = componentSearch.getProcessGroupFlow().getFlow().getInputPorts().stream().filter(port -> name.equals(port.getComponent().getName())).findFirst()
                    .map(p -> {
                        // BLOCKTEST EVAL: https://github.com/hermannpencole/nifi-config/blob/e4a8a251bd01fd4026ac4fe6868791f4496ce882/src/main/java/com/github/hermannpencole/nifi/config/service/ConnectionsUpdater.java#L101-L107
                        blocktest().given(componentSearch, new ProcessGroupFlowEntity()).given(p, new PortEntity().id("bar")).setup(() -> {
                            componentSearch.setProcessGroupFlow(new ProcessGroupFlowDTO().id("foo"));
                        }).checkEq(((ConnectableDTO) methodReturn).getId(), "bar");

                        blocktest().given(componentSearch, new ProcessGroupFlowEntity()).given(p, new PortEntity().id("www")).setup(() -> {
                            componentSearch.setProcessGroupFlow(new ProcessGroupFlowDTO().id("baz"));
                        }).checkEq(((ConnectableDTO) methodReturn).getId(), "www").checkEq(((ConnectableDTO) methodReturn).getType(), ConnectableDTO.TypeEnum.INPUT_PORT).checkEq(((ConnectableDTO) methodReturn).getGroupId(), "baz");
        ...
        if (!result.isPresent())
            result = componentSearch.getProcessGroupFlow().getFlow().getOutputPorts().stream().filter(port -> name.equals(port.getComponent().getName())).findFirst()
                    .map(p -> {
                        // BLOCKTEST EVAL: https://github.com/hermannpencole/nifi-config/blob/e4a8a251bd01fd4026ac4fe6868791f4496ce882/src/main/java/com/github/hermannpencole/nifi/config/service/ConnectionsUpdater.java#L110-L116
                        blocktest().given(componentSearch, new ProcessGroupFlowEntity()).given(p, new PortEntity().id("bar")).setup(() -> {
                            componentSearch.setProcessGroupFlow(new ProcessGroupFlowDTO().id("foo"));
                        }).checkEq(((ConnectableDTO) methodReturn).getId(), "bar");
                        ...
                        blocktest().given(componentSearch, new ProcessGroupFlowEntity()).given(p, new PortEntity().id("www")).setup(() -> {
                            componentSearch.setProcessGroupFlow(new ProcessGroupFlowDTO().id("baz"));
                        }).checkEq(((ConnectableDTO) methodReturn).getId(), "www").checkEq(((ConnectableDTO) methodReturn).getType(), ConnectableDTO.TypeEnum.OUTPUT_PORT).checkEq(((ConnectableDTO) methodReturn).getGroupId(), "baz");
                        ConnectableDTO connectableDTO = new ConnectableDTO();
                        connectableDTO.setGroupId(componentSearch.getProcessGroupFlow().getId());
                        connectableDTO.setId(p.getId());
       ...
       /* if (!result.isPresent())
        ...
        if (!result.isPresent())
            result = componentSearch.getProcessGroupFlow().getFlow().getRemoteProcessGroups().stream().filter(remoteProcessGroup -> name.equals(remoteProcessGroup.getComponent().getName())).findFirst()
                    .map(p -> {
                        // BLOCKTEST EVAL: https://github.com/hermannpencole/nifi-config/blob/e4a8a251bd01fd4026ac4fe6868791f4496ce882/src/main/java/com/github/hermannpencole/nifi/config/service/ConnectionsUpdater.java#L121-L127
                        blocktest().given(componentSearch, new ProcessGroupFlowEntity()).given(p, new RemoteProcessGroupEntity().id("bar")).setup(() -> {
                            componentSearch.setProcessGroupFlow(new ProcessGroupFlowDTO().id("foo"));
                        }).checkEq(((ConnectableDTO) methodReturn).getId(), "bar");

                        blocktest().given(componentSearch, new ProcessGroupFlowEntity()).given(p, new RemoteProcessGroupEntity().id("www")).setup(() -> {
                            componentSearch.setProcessGroupFlow(new ProcessGroupFlowDTO().id("baz"));
                        }).checkEq(((ConnectableDTO) methodReturn).getId(), "www").checkEq(((ConnectableDTO) methodReturn).getType(), ConnectableDTO.TypeEnum.PROCESSOR).checkEq(((ConnectableDTO) methodReturn).getGroupId(), "baz");
                        ConnectableDTO connectableDTO = new ConnectableDTO();
                        connectableDTO.setGroupId(componentSearch.getProcessGroupFlow().getId());
                        connectableDTO.setId(p.getId());
                        connectableDTO.setType(ConnectableDTO.TypeEnum.PROCESSOR);
