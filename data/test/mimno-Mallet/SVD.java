// TEST_DIR: src/test/java/cc/mallet/util
        ...
        for (int col = 0; col < maxCol; col++) {
            ...
            for (int otherCol = col + 1; otherCol < numCols; otherCol++) {
                if (col < numRows - 1 && diagonal[col] != 0.0) {
                    ...
                    // BLOCKTEST EVAL: https://github.com/mimno/Mallet/blob/ff204eb8aa7f414e779f37843fb68b3e1fdc7015/src/cc/mallet/util/SVD.java#L69C1-L78C22
                    blocktest().given(col, 0).given(numRows, 2).given(sum, 0.0)
                            .given(this.originalMatrix, new double[][] {{0.0, 1.0}, {2.0, 3.0}}).given(otherCol, 1)
                            .checkEq(sum, 6.0, Double.MIN_VALUE).start(FIRST_BLOCK, 4);
                }
        ...
        for (int col = maxCol - 1; col >= 0; col --) {
            if (diagonal[col] != 0.0) {
                ...
                for (int otherCol = col + 1; otherCol < numCols; otherCol++) {
                    ...
                    // BLOCKTEST EVAL: https://github.com/mimno/Mallet/blob/ff204eb8aa7f414e779f37843fb68b3e1fdc7015/src/cc/mallet/util/SVD.java#L161C1-L168C22
                    blocktest().given(col, 0).given(numRows, 2).given(sum, 0.0)
                            .given(this.U, new double[][] {{1.0, 2.0}, {3.0, 4.0}}).given(otherCol, 1)
                            .checkEq(sum, 14.0, Double.MIN_VALUE).start(FIRST_BLOCK, 4);
                    blocktest().given(col, 0).given(numRows, 2).given(scale, -7.0)
                            .given(this.U, new double[][] {{1.0, 2.0}, {3.0, 4.0}}).given(otherCol, 1)
                            .checkEq(scale, -14.0, Double.MIN_VALUE).start(FIRST_BLOCK, 4);
                }
            ...
            if (situation == 1) {
                ...
                for (int col = startCol; col < lastCol - 1; col++) {
                    ...
                    for (int row = 0; row < numRows; row++) {
                        ...
                        U[row][col] = temp;
                        // BLOCKTEST EVAL: https://github.com/mimno/Mallet/blob/ff204eb8aa7f414e779f37843fb68b3e1fdc7015/src/cc/mallet/util/SVD.java#L415C1-L417C44
                        blocktest().given(col, 0).given(numRows, 2)
                                .given(this.U, new double[][] {{1.0, 2.0}, {3.0, 4.0}}).given(row, 1)
                                .given(cosine, 1.0).given(sine, 0.0)
                                .checkEq(temp, 3.0, Double.MIN_VALUE).start(FIRST_BLOCK, 4);
                    }
                }
