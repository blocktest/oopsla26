        ...
        if (!this.customFilters.isEmpty()) {
            for (Map.Entry<String, Input<Serializable>> entry : customFilters.entrySet()) {
                ...
                if (filter != null) {
                    ...
                    // BLOCKTEST EVAL: https://github.com/adobe/commerce-cif-magento-graphql/blob/0bc22bb767a8270f94ee04d85ab2571569a146a1/src/main/java/com/adobe/cq/commerce/magento/graphql/ProductSortInput.java#L1270C1-L1275C22
                    // Case good
                    blocktest().given(filter, new CreateCompareListInput()).given(_queryBuilder, new StringBuilder(""))
                            .checkEq(_queryBuilder.toString(), "{}").start(FIRST_BLOCK);
                    // Case bad
                    blocktest().given(filter, "foo").given(_queryBuilder, new StringBuilder(""))
                            .checkEq(_queryBuilder.toString(), "null").start(FIRST_BLOCK);
                } else {
                    _queryBuilder.append("null");
                }
            }
