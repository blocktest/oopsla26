                    ...
                    if (rootWord && root.duplicatesDuringSuffixation() && !startState.getName().startsWith("VerbalRoot") && TurkishLanguage.isConsonantDrop(with.charAt(0))) {
                        ...
                        if (rootWord && root.lastIdropsDuringSuffixation() && !startState.getName().startsWith("VerbalRoot") && !startState.getName().startsWith("ProperRoot") && startWithVowelorConsonantDrops()) {
                            ...
                            if (softenDuringSuffixation(root)) {
                                ...
                                // BLOCKTEST EVAL: https://github.com/StarlangSoftware/TurkishMorphologicalAnalysis/blob/26450b47db92b64580a2bfa4a0d7d0bce38edd49/src/main/java/MorphologicalAnalysis/Transition.java#L317C1-L329C47
                                blocktest().given(formation, "").given(stem, "hizip").checkEq(formation, "hizb");
                                blocktest().given(formation, "").given(stem, "kayıp\'").checkEq(formation, "kayıb");
                                blocktest().given(formation, "").given(stem, "akit").checkEq(formation, "akd");
                                blocktest().given(formation, "").given(stem, "ahit\'").checkEq(formation, "ahid");
                                blocktest().given(formation, "").given(stem, "eviç").checkEq(formation, "evc");
                                blocktest().given(formation, "").given(stem, "nesiç\'").checkEq(formation, "nesic");
                                blocktest().given(formation, "").given(stem, "dummy").checkEq(formation, "");
                                blocktest().given(formation, "").given(stem, "").checkEq(formation, "");
                                switch (Word.lastPhoneme(stem)) {
                                    case 'p':
