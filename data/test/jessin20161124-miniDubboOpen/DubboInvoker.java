    ...
    public Object invoke(RpcInvocation rpcInvocation) {
        ...
        if (CompletableFuture.class.isAssignableFrom(rpcInvocation.getMethod().getReturnType())) {
            ...
            defaultFuture.addCallback((result, exception) -> {
                // BLOCKTEST EVAL: https://github.com/jessin20161124/miniDubboOpen/blob/a02a5c2b393456861401b14057752fb22a7308ab/src/main/java/com/jessin/practice/dubbo/invoker/DubboInvoker.java#L55-L61
                blocktest().given(result, "foo").given(ret, new CompletableFuture()).given(exception, new FileNotFoundException()).checkTrue(ret.isCompletedExceptionally());
                blocktest().given(result, "foo").given(ret, new CompletableFuture()).given(exception, null).checkTrue(ret.isDone());
                if (exception != null) {
