        ...
        try {
            ...
            if (responseData != null && CompletableFuture.class.isAssignableFrom(method.getReturnType())) {
                ...
                ((CompletableFuture) responseData).whenComplete((result, exception) -> {
                    // BLOCKTEST EVAL: https://github.com/jessin20161124/miniDubboOpen/blob/a02a5c2b393456861401b14057752fb22a7308ab/src/main/java/com/jessin/practice/dubbo/netty/NettyServerHandler.java#L103-L112
                    blocktest().given(exception, new FileNotFoundException()).given(response, new Response()).given(result, null).mock("ctx.writeAndFlush(..)")
                            .checkTrue(response.isException()).checkTrue(response.getResult().toString().contains("java.io.FileNotFoundException"));
                    blocktest().given(exception, null).given(response, new Response()).given(result, "Test").mock("ctx.writeAndFlush(..)")
                            .checkFalse(response.isException()).checkEq(response.getResult().toString(), "Test");
                    if (exception != null) {
                        response.setException(true);
