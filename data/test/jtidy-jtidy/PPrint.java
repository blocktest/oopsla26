...
public class PPrint
{
    ...
    private void printChar(int c, short mode)
    {
        ...
        if ("UTF8".equals(this.configuration.getOutCharEncodingName()))
        {
            ...
            {
                // BLOCKTEST EVAL: https://github.com/jtidy/jtidy/blob/aade7c5a6c1007d013859ac95378a2d469f5e254/src/main/java/org/w3c/tidy/PPrint.java#L737C1-L778C18
                blocktest().given(c, 0x2000).given(linelen, 0).given(wraphere, 0).given(breakable, false).checkEq(wraphere, 1).checkTrue(breakable).end(FIRST_BLOCK);
                blocktest().given(c, 0x2007).given(linelen, 0).given(wraphere, 0).given(breakable, false).checkEq(wraphere, 0).checkFalse(breakable).end(FIRST_BLOCK);
                blocktest().given(c, 0xFE63).given(linelen, 0).given(wraphere, 0).given(breakable, false).checkEq(wraphere, 1).checkTrue(breakable).end(FIRST_BLOCK);
        ...
        if ("ISO8859_1".equals(this.configuration.getOutCharEncodingName()))
        {
            if (c > 255) /* multi byte chars */
            {
                if (!this.configuration.numEntities)
                {
                  ...
                  // BLOCKTEST EVAL: https://github.com/jtidy/jtidy/blob/aade7c5a6c1007d013859ac95378a2d469f5e254/src/main/java/org/w3c/tidy/PPrint.java#L904C17-L912C22
                  blocktest().given(c, 8364).checkEq(entity, "&euro;").start(FIRST_BLOCK, 2);
                  blocktest().given(c, 0).checkEq(entity, "&#0;").start(FIRST_BLOCK, 2);
                }
                else
                {
                    entity = "&#" + c + ";";
