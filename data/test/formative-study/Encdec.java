// BLOCKTEST EXAMPLE: https://github.com/codelibs/jcifs/blob/c9ad4eeaf763ad764cb485c4149a0b236eaa7d3e/src/main/java/jcifs/util/Encdec.java#L318-L328
    ...
    public static String dec_utf8 ( byte[] src, int si, int slim ) throws IOException {
        ...
        for ( ui = 0; si < slim && ( ch = src[ si++ ] & 0xFF ) != 0; ui++ ) {
            if ( ch < 0x80 ) {
                ...
                blocktest().given(si, 0).given(ch, 0xF0).given(ui, 0).given(src, new byte[]{'a', 'b', 'c', 'd'}).given(uni, new char[]{'a', 'b', 'c', 'd'}).expect(IOException.class);
