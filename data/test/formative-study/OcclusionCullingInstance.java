// BLOCKTEST UNSUPPORTED EXAMPLE: https://github.com/LogisticsCraft/OcclusionCulling/blob/7a346eabc5edcfdef31df3c21202c3c1473fb506/src/main/java/com/logisticscraft/occlusionculling/OcclusionCullingInstance.java#L120C25-L137
    ...
    try {
      ...
      for (int x = minX; x <= maxX; x++) {
        ...
        for (int y = minY; y <= maxY; y++) {
          ...
          for (int z = minZ; z <= maxZ; z++) {
            blocktest().given(faceEdgeDataY, 0).given(visibleOnFaceY, 0).given(z, 0).given(minZ, 0).given(minZ, 1).given(id, 0).checkEq(id, 1);
