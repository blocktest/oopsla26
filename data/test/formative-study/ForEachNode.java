// BLOCKTEST EXAMPLE: https://github.com/mvel/mvel/blob/bac95e0cf8884a0713693161c589514a8a43fe87/src/main/java/org/mvel2/ast/ForEachNode.java#L181-L189
/**
	* MVEL 2.0
...
public class ForEachNode extends BlockNode {
	...
	private void handleCond(char[] condition, int start, int offset, int fields, ParserContext pCtx) {
		...
		if ((x = (item = createStringTrimmed(condition, start, cursor - start)).indexOf(' ')) != -1) {
			blocktest().given(condition, "for (String s : strings) {}".toCharArray()).given(start, 5).given(x, 6).given(cursor, 14).given(pCtx, null).checkEq(itemType.toString(), "class java.lang.String");
			// blocktest().given(condition, "for (   String   s : strings ) {}".toCharArray()).given(start, 5).given(x, 6).given(cursor, 19).given(pCtx, null).checkEq(itemType.toString(), "class java.lang.String");
			// above blocktest will fail due to a bug in the code below: https://github.com/mvel/mvel/commit/f33227ec8f785b97cc69c7f58a097fd05d259fc4
