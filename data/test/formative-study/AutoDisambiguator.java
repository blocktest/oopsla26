// BLOCKTEST EXAMPLE: https://github.com/StarlangSoftware/TurkishMorphologicalDisambiguation/blob/a2773f47116b17db1660359925b45454e20f639f/src/main/java/MorphologicalDisambiguation/AutoDisambiguator.java#L536C1-L552C30
  ...
  private static String selectCaseForParseString(String parseString, int index, FsmParseList[] fsmParses, ArrayList<FsmParse> correctParses) {
    ...
    switch (parseString) {
      ...
      case "ADJ$ADV$VERB+POS+IMP+A2SG":
        // @blocktest("singleCommentTest").given(surfaceForm, "sık").given(index, 1).setup("Sentence sentence = new Sentence(\"Yarın doktora gidecekler\");FsmMorphologicalAnalyzer fsm = new FsmMorphologicalAnalyzer();fsmParses = fsm.robustMorphologicalAnalysis(sentence);").checkEq(methodReturn, "ADV");
    
        /*
          @blocktest("methodReturnTest").given(surfaceForm, "sık").given(index, 1).setup(() -> {
            Sentence sentence = new Sentence("Yarın doktora gidecekler");
            FsmMorphologicalAnalyzer fsm = new FsmMorphologicalAnalyzer();
            fsmParses = fsm.robustMorphologicalAnalysis(sentence);
          }).checkEq(methodReturn, "ADV");
        */
      
        /*
          @blocktest("checkReturnEqTest").given(surfaceForm, "sık").given(index, 1).setup(() -> {
            Sentence sentence = new Sentence("Yarın doktora gidecekler");
            FsmMorphologicalAnalyzer fsm = new FsmMorphologicalAnalyzer();
            fsmParses = fsm.robustMorphologicalAnalysis(sentence);
          }).checkReturnEq("ADV");
