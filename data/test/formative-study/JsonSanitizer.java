// BLOCKTEST EXAMPLE: https://github.com/OWASP/json-sanitizer/blob/fc612ab374de73d03864d56fb87b6a103b234489/src/main/java/com/google/json/JsonSanitizer.java#L342C18-L351C25
          ...
          case '/':
            ...
            if (i + 1 < n) {
              switch (jsonish.charAt(i + 1)) {
                case '/':
                  blocktest().given(jsonish, "abc\nde").given(n, 6).given(i, 0).checkEq(end, 4);
                  end = n;  // Worst case.
                  for (int j = i + 2; j < n; ++j) {
      ...
      switch (ch) {
        ...
        case '"': case '\'':
          if (i == start) {
            ...
            if (i + 1 == end) {
              blocktest().given(jsonish, "helloworld\"").given(start, 0).given(ch, '"').checkTrue(closed);
              blocktest().given(jsonish, "helloworld\'").given(start, 0).given(ch, '\'').checkFalse(closed);
        ...
        case '<': {
          ...
          // @blocktest().given(jsonish, "hi<s\\cript>test</script>").given(i, 2).checkEq(lc1, 's').checkEq(lc2, 'c').checkEq(lc3, 'r').end(FIRST_BLOCK, false);
          blocktest().given(jsonish, "hi<s\\cript>test</script>").given(i, 2).checkEq("lc1", 's').checkEq("lc2", 'c').checkEq("lc3", 'r').end(FIRST_BLOCK, false);
          int la = i + 1;
          ...
          char lc2 = (char) (c2 | 32);
          char lc3 = (char) (c3 | 32);
          blocktest().given(jsonish, "hi<s\\cript>test</script>").given(i, 2).checkEq(lc1, 's').checkEq(lc2, 'c').checkEq(lc3, 'r').start(FIRST_BLOCK, false);
          if (
                  (c1 == '!' && c2 == '-' && c3 == '-') ||
                          (lc1 == 's' && lc2 == 'c' && lc3 == 'r') ||
                          (c1 == '/' && lc2 == 's' && lc3 == 'c')
  ...
  private void normalizeNumber(int start, int end) {
    ...
    if (pos == intEnd) {  // No empty integer parts allowed in JSON.
      ...
      if (intEnd - pos == 1 && intEnd < end
        ...
        blocktest().given(maxDigVal, 0).given(jsonish, "2652a0932").given(intEnd, 0).given(end, 9).checkEq(maxDigVal, 10);
        blocktest().given(maxDigVal, 0).given(jsonish, "265200932").given(intEnd, 0).given(end, 9).checkEq(maxDigVal, 9);
  ...
  private static int unescapedChar(String s, int left) {
    ...
    if (c == '\\') {
      ...
      switch (nc) {
        ...
        case 'u':
          if (left + 5 < n) {
            // blocktest().given(left, 0).given(s, "helloworld"); // what to assert?
            char d0 = s.charAt(left + 2);
            char d1 = s.charAt(left + 3);
