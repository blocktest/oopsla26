// BLOCKTEST EXAMPLE: https://github.com/AgNO3/jcifs-ng/blob/a9950dbcd61de695b14cc1fe21e199737446ada6/src/main/java/jcifs/smb/SmbCopyUtil.java#L241-L255
            ...
            try ( SmbFileHandleImpl sfd = src.openUnshared(0, SmbConstants.O_RDONLY, SmbConstants.FILE_SHARE_READ, SmbConstants.ATTR_NORMAL, 0) ) {
                ...
                do {
                    ...
                    while ( ooff < size ) {
                        ...
                        // Since maxChunks and maxChunkSize are constants, ideally, blocktest should automatically find and declare these two variables
                      blocktest().given(maxChunks, 256).given(maxChunkSize, 1024 * 1024).given(wsize, 1048576*256+2).checkEq("lastChunkSize", 1048576).end(FIRST_BLOCK);
//                      blocktest().given(wsize, 1048576*2+1).checkEq("lastChunkSize", 1).end(FIRST_BLOCK);
//                      
//                      blocktest().given(wsize, 1048576*256+2).given(maxChunks, 256).given(maxChunkSize, 1024 * 1024).checkEq("lastChunkSize", 1048576).end(FIRST_BLOCK);
//                      blocktest().given(wsize, 1048576*2+1).given(maxChunks, 256).given(maxChunkSize, 1024 * 1024).checkEq("lastChunkSize", 1).end(FIRST_BLOCK);
