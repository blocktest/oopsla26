...
//            word0(d2) -= 31*Exp_msk1; /* adjust exponent */
        ...
        if (ilim >= 0 && ilim <= Quick_max && try_quick) {
            ...
            if (k > 0) {
                ...
                // BLOCKTEST EVAL: https://github.com/erdtman/java-json-canonicalization/blob/535ab0075405f6aafb96a4197f943838130666d3/src/main/java/org/erdtman/jcs/NumberDToA.java#L461C4-L466C25
                blocktest().given(j, 5).given(i, 0).given(ieps, 0).given(ds, 1).given(d, 1e80).checkEq(d, 1.0, 0.01).checkEq(ieps, 2);
                for (; (j != 0); j >>= 1, i++)
                    if ((j & 1) != 0) {
                        ieps++;
                        ds *= bigtens[i];
                ...
                // BLOCKTEST EVAL: https://github.com/erdtman/java-json-canonicalization/blob/535ab0075405f6aafb96a4197f943838130666d3/src/main/java/org/erdtman/jcs/NumberDToA.java#L468C1-L473C22
                blocktest().given(j, 25).given(j1, 100).given(i, 0).given(ieps, 0).given(ds, 1).given(d, 1e2).checkEq(d, 1e102, 0.01).checkEq(ieps, 2);
                d *= tens[j1 & 0xf];
...
//                            goto bump_up;
                            char lastCh;
                            // BLOCKTEST EVAL: https://github.com/erdtman/java-json-canonicalization/blob/535ab0075405f6aafb96a4197f943838130666d3/src/main/java/org/erdtman/jcs/NumberDToA.java#L523C1-L532C30
                            blocktest().given(buf, new StringBuilder().append("5599")).given(k, 0).checkEq(buf.toString(), "56").checkEq(k, 0).checkReturnEq(1);
                            blocktest().given(buf, new StringBuilder().append("999")).given(k, 0).checkEq(buf.toString(), "1").checkEq(k, 1).checkReturnEq(2);
                            while (true) {
...
//                        goto round_9_up;
                ...
                if ((j < 0)
                    ...
                    if (j1 > 0) {
                        ...
                        // BLOCKTEST EVAL: https://github.com/erdtman/java-json-canonicalization/blob/535ab0075405f6aafb96a4197f943838130666d3/src/main/java/org/erdtman/jcs/NumberDToA.java#L826-L841
                        blocktest().given(j1, 0).given(b, new BigInteger("1000")).given(S, new BigInteger("300")).given(buf, new StringBuilder().append("999")).given(biasUp, true).given(dig, '9').given(k, 0)
                                .checkEq(k, 1).checkEq(buf.toString(), "1");
                        b = b.shiftLeft(1);
                        j1 = b.compareTo(S);
