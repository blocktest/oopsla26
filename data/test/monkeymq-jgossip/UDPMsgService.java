    ...
    public void unListen() {
        if (socket != null) {
            socket.close(asyncResult -> {
                // BLOCKTEST EVAL: https://github.com/monkeymq/jgossip/blob/4bfc8e791381f70d62ea4ad6d8ae952da840dd0d/src/main/java/net/lvsq/jgossip/net/udp/UDPMsgService.java#L96-L102
                blocktest().given(asyncResult, new AsyncResult<Void>() {
                    @Override
                    public Void result() {
                        return null;
                    }
                ...
                }).mock("LOGGER.info(..)").mock("LOGGER.error(..)").checkFlow(IfStmt().Then());
                blocktest().given(asyncResult, new AsyncResult<Void>() {
                    @Override
                    public Void result() {
                        return null;
                ...
                }).mock("LOGGER.info(..)").mock("LOGGER.error(..)").checkFlow(IfStmt().Else());
                if (asyncResult.succeeded()) {
