    ...
    private String getAuthority(String scheme, boolean maskCredentials) {
        ...
        if (sentinels.size() != 0) {
            ...
            authority = sentinels.stream().map(redisURI -> {
                // BLOCKTEST EVAL: https://github.com/lettuce-io/lettuce-core/blob/761d60247159662ce1db8bc20318bc4b573bc010/src/main/java/io/lettuce/core/RedisURI.java#L925-L931
                blocktest().given(redisURI, new RedisURI("foo", 80, Duration.ofDays(1))).given(scheme, "https").mock("getPortPart(..)", "12").checkReturnEq("foo12");
                blocktest().given(redisURI, new RedisURI("foo", 80, Duration.ofDays(1))).given(scheme, "https").mock("getPortPart(..)", "12").setup( () -> {
                        redisURI.setSocket("123A");
                    }
                ).checkReturnEq("[Socket 123A]");
