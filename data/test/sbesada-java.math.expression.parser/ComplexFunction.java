		...
		for (int i = 0; i < f.length(); i++) {
			...
			if (character >= '0' && character <= '9') {
				...
				if (hasNumber && !isImaginary) {
					// BLOCKTEST EVAL: https://github.com/sbesada/java.math.expression.parser/blob/cc92f58f6074b21cf6f5d4056b7f7cc5168f50af/src/main/java/com/expression/parser/function/ComplexFunction.java#L102C2-L107C18
					blocktest().given(number, "10.5").given(i, 5).given(f, "abcdefghi")
							.mock("eval(..)", new Complex(5, 1))
							.checkEq(value.getR(), 15.5, 0.001)
							.checkEq(value.getI(), 1, 0.001)
							.checkEq(i, 8)
							.checkEq("new_f", "ghi")
							.checkFalse(hasNumber).checkEq(number, "");
