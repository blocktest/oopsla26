            ...
            switch (section) {
                ...
                case "head":
                    if (sectionToken == JsonToken.START_OBJECT) {
                        // BLOCKTEST EVAL: https://github.com/datadotworld/dw-jdbc/blob/ac4f7d8a457dce89cbaa9eb33994304e5504fda1/src/main/java/world/data/jdbc/internal/transport/SparqlResultsParser.java#L67-L76
                        lambdatest().noInit(parser).given(foundVariables, new boolean[]{false}).args("vars", JsonToken.START_ARRAY)
                                .mock("ElementParser.parseArray(..)").checkTrue(foundVariables[0]);
                        lambdatest().noInit(parser).given(foundVariables, new boolean[]{false}).args("vars2", JsonToken.START_ARRAY)
                                .mock("ElementParser.parseArray(..)").checkFalse(foundVariables[0]);
                        lambdatest().noInit(parser).given(foundVariables, new boolean[]{false}).args("vars", JsonToken.END_OBJECT)
                                .mock("ElementParser.parseArray(..)").checkFalse(foundVariables[0]);
