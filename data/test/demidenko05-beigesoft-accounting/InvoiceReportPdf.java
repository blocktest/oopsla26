...
import org.beigesoft.accounting.persistable.IdI18nCurrency;import org.beigesoft.persistable.Languages;import org.blocktest.BTest;
...
public class InvoiceReportPdf<RS, WI>
  ...
  public final void makeReport(final Map<String, Object> pReqVars,
    ...
    for (SalesInvoiceTaxLine itl : inv.getTaxesLines()) {
      if (itl.getTaxableInvBas().compareTo(BigDecimal.ZERO) == 1) {
        isInvoiceBasis = true;
        break;
      }
    }
    ...
    String priceName;
    if (inv.getPriceIncTax()) {
      priceName = "priceIncTax";
    } else {
      priceName = "itsPrice";
    }
    ...
    if (!lang.equals(langDef)) {
      ...
      // BLOCKTEST EVAL: https://github.com/demidenko05/beigesoft-accounting/blob/0170596e736d3cc69f70c88ced3b9d78f63ddca1/src/main/java/org/beigesoft/accounting/report/InvoiceReportPdf.java#L142C6-L148C8
      blocktest().given(i18nCurrency, null).setup(() -> {
              ...
              }).given(i18nCurrencyLst, new java.util.ArrayList<I18nCurrency>()).given(lang, "baz2").given(currency, new Currency())
              .checkTrue(i18nCurrency != null).checkTrue(i18nCurrency.getHasName().getItsId() == 456L).checkEq(i18nCurrency.getLang().getItsName(), "bar");
    ...
    if (inv.getServices() != null && inv.getServices().size() > 0) {
      ...
      pos++;
      tblServices.getItsCells().get(pos)
              .setItsContent(this.srvI18n.getMsg("taxesDescription", lang));
      if (!isInvoiceBasis) {
