    ...
    private static void setTermsAggregation(SearchSourceBuilder searchSource, TermsAggregation aggregation, Integer size) {
        ...
        if (aggregation.order().length > 0) {
            List<BucketOrder> orders = Stream.of(aggregation.order()).distinct().map(t -> {
                ...
                blocktest().given(t, TermsAggregation.Order.COUNT_ASC).checkReturnEq(BucketOrder.count(Boolean.TRUE));
                blocktest().given(t, TermsAggregation.Order.COUNT_DESC).checkReturnEq(BucketOrder.count(Boolean.FALSE));
                blocktest().given(t, TermsAggregation.Order.KEY_ASC).checkReturnEq(BucketOrder.key(Boolean.TRUE));
                blocktest().given(t, TermsAggregation.Order.KEY_DESC).checkReturnEq(BucketOrder.key(Boolean.FALSE));
                switch (t) {
                    case COUNT_ASC:
