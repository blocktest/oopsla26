    ...
    // Merge individual properties in style only using the strategy when both elements have a value.
      ...
      if (ConflictStrategy.USE_BEAN.equals(strategy)) {
        propertyNameStream = propertyNameStream
                .filter(propertyName -> {
                  // BLOCKTEST EVAL: https://github.com/errai/errai/blob/d2edc4632373096f8d81c93f18cd874fa5501b90/errai-ui/src/main/java/org/jboss/errai/ui/shared/TemplateUtil.java#L181-L184
                  /*
                  @blocktest().given(beanElement, null, "HTMLElement").given(propertyName, "color").setup(() -> {
                    beanElement = new Widget().getElement().cast();
                    beanElement.getStyle().setProperty("color", "red");
