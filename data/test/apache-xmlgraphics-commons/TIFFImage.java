...
public class TIFFImage extends AbstractRed {
    ...
    // Image types
    static final int TYPE_UNSUPPORTED = -1;

    static final int TYPE_BILEVEL = 0;

    static final int TYPE_GRAY_4BIT = 1;

    static final int TYPE_GRAY = 2;

    static final int TYPE_GRAY_ALPHA = 3;

    static final int TYPE_PALETTE = 4;

    static final int TYPE_RGB = 5;

    static final int TYPE_RGB_ALPHA = 6;

    static final int TYPE_YCBCR_SUB = 7;

    static final int TYPE_GENERIC = 8;

    // Incidental tags
    static final int TIFF_JPEG_TABLES = 347;

    static final int TIFF_YCBCR_SUBSAMPLING = 530;
    ...
    public TIFFImage(SeekableStream stream, TIFFDecodeParam param, int directory) throws IOException {
        ...
        // BLOCKTEST EVAL: https://github.com/apache/xmlgraphics-commons/blob/d5d782f9fce99774ef206a4a4437fc86f711d9d9/src/main/java/org/apache/xmlgraphics/image/codec/tiff/TIFFImage.java#L216C6-L231C43
        blocktest().noInit(dir).mock("dir.getField(..)", new TIFFField(1, 2, 3, new char[]{100, 101, 102}))
                .checkEq(sampleSize, 100).end(FIRST_ASSIGN);
        blocktest().noInit(dir).mock("dir.getField(..)", null)
                .checkEq(sampleSize, 1).end(FIRST_ASSIGN);
        ...
        int photometricType;
        TIFFField photometricTypeField = dir.getField(TIFFImageDecoder.TIFF_PHOTOMETRIC_INTERPRETATION);
    ...
    public synchronized Raster getTile(int tileX, int tileY) {
        ...
        if (!tiled) {
            ...
            newRect = new Rectangle(tile.getMinX(), tile.getMinY(), tileWidth, tileHeight);
        }
            ...
            if (sampleSize == 16) {
                if (decodePaletteAsShorts) {
                    ...
                    if (dataType == DataBuffer.TYPE_USHORT) {
                        ...
                        }).checkTrue(sdata[0] == 11 && sdata[1] == 4 && sdata[2] == 1 && sdata[3] == 12);
                ...
                if (decodePaletteAsShorts) {
                    ...
                    try {
                        if (compression == COMP_PACKBITS) {
                            ...
                            throw new RuntimeException(PropertyUtil.getString("IFFImage10") + ": " + compression);
                        }
                        stream.seek(saveOffset);
                    } catch (IOException ioe) {
                        throw new RuntimeException(PropertyUtil.getString("TIFFImage13") + ": " + ioe.getMessage());
