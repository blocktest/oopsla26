    ...
    private int encode(RenderedImage im, TIFFEncodeParam encodeParam,
        ...
        if (compression == CompressionValue.NONE) {
            ...
            if (!isLast) {
                ...
                // BLOCKTEST EVAL: https://github.com/apache/xmlgraphics-commons/blob/d5d782f9fce99774ef206a4a4437fc86f711d9d9/src/main/java/org/apache/xmlgraphics/image/codec/tiff/TIFFImageEncoder.java#L448C1-L454C18
                blocktest().given(totalBytesOfData, 2048).given(skipByte, false).given(tileOffsets, new long[]{1000L}).checkEq(nextIFDOffset, 3048).checkFalse(skipByte);
                blocktest().given(totalBytesOfData, 2048).given(skipByte, false).given(tileOffsets, new long[]{1001L}).checkEq(nextIFDOffset, 3050).checkTrue(skipByte);
                blocktest().given(totalBytesOfData, 1024).given(skipByte, false).given(tileOffsets, new long[]{0L}).checkEq(nextIFDOffset, 1024).checkFalse(skipByte);
                blocktest().given(totalBytesOfData, 1023).given(skipByte, false).given(tileOffsets, new long[]{0L}).checkEq(nextIFDOffset, 1024).checkTrue(skipByte);
                blocktest().given(totalBytesOfData, 25000).given(skipByte, false).given(tileOffsets, new long[]{50000L}).checkEq(nextIFDOffset, 75000).checkFalse(skipByte);
                blocktest().given(totalBytesOfData, 25000).given(skipByte, false).given(tileOffsets, new long[]{50001L}).checkEq(nextIFDOffset, 75002).checkTrue(skipByte);
                blocktest().given(totalBytesOfData, 1).given(skipByte, false).given(tileOffsets, new long[]{0L}).checkEq(nextIFDOffset, 2).checkTrue(skipByte);
                blocktest().given(totalBytesOfData, 1001).given(skipByte, false).given(tileOffsets, new long[]{1001L}).checkEq(nextIFDOffset, 2002).checkFalse(skipByte);
