    ...
    private double eval(String f_x, final double xi) throws CalculatorException {
        ...
        for (int i = 0; i < f_x.length(); i++) {
            ...
            case '9':
                ...
                break;
            case '.':
                // BLOCKTEST EVAL: https://github.com/sbesada/java.math.numerical.library/blob/a2f18eda6de69c937183771d3b1f268e606a8871/src/main/java/com/mathlibrary/function/FunctionX.java#L321C1-L326C18
                blocktest().given(hasNumber, true).given(number, "123").given(i, 5).given(f_x, "123456").given(character, '4').expect(CalculatorException.class);
                blocktest().given(hasNumber, true).given(number, "123").given(i, 5).given(f_x, "1234567").given(character, '4').checkEq(number, "1234");
                blocktest().given(hasNumber, false).given(number, "123").given(i, 5).given(f_x, "1234567").given(character, 'x').checkEq(number, "123");
                blocktest().given(hasNumber, true).given(number, "").given(i, 5).given(f_x, "1234567").given(character, 'x').checkEq(number, "");
                if (i == (f_x.length() - 1)) {
