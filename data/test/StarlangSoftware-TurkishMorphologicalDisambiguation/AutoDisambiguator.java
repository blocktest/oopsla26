    ...
    private static String selectCaseForParseString(String parseString, int index, FsmParseList[] fsmParses, ArrayList<FsmParse> correctParses) {
        ...
        switch (parseString) {
            ...
            case "A3PL+PNON+NOM$A3SG+PNON+NOM^DB+VERB+ZERO+PRES+A3PL$PROP+A3PL+PNON+NOM":
                // BLOCKTEST EVAL: https://github.com/StarlangSoftware/TurkishMorphologicalDisambiguation/blob/bf8dbc2cd835c67178eb896939ad1009b6743486/src/main/java/MorphologicalDisambiguation/AutoDisambiguator.java#L275-L281
                /*
                    @blocktest("test1").given(index, 1).setup(() -> {
                        Sentence sentence = new Sentence("Yarın doktora gidecekler");
                        FsmMorphologicalAnalyzer fsm = new FsmMorphologicalAnalyzer();
                        fsmParses = fsm.robustMorphologicalAnalysis(sentence);
                    }).checkReturnEq("A3PL+PNON+NOM");
                 */
                /*
                    @blocktest("test2").given(index, 1).setup(() -> {
                        Sentence sentence = new Sentence("Yarın Telerate gidecekler");
                        FsmMorphologicalAnalyzer fsm = new FsmMorphologicalAnalyzer();
                        fsmParses = fsm.robustMorphologicalAnalysis(sentence);
                    }).checkReturnEq("PROP+A3PL+PNON+NOM");
                 */
                /*
                    @blocktest("test3").given(index, 0).setup(() -> {
                        Sentence sentence = new Sentence("Yarın Doktora gidecekler");
                        FsmMorphologicalAnalyzer fsm = new FsmMorphologicalAnalyzer();
                        fsmParses = fsm.robustMorphologicalAnalysis(sentence);
                    }).checkFlow(IfStmt());
