    ...
    private void checkUnknown(int position) throws InterruptedException {
        ...
        for (int w0 = configuration.getKnownStart(); RESULT == null && w0 < DICTIONARY_SIZE; w0++) {
            ...
            for (int t = 0; t < THREADS; t++) {
                ...
                // BLOCKTEST EVAL: https://github.com/PawelGorny/lostword/blob/8e66ab8737a5119a21cef6c948ae7b48f00fdb91/src/main/java/com/pawelgorny/lostword/WorkerKnownPosition.java#L87-L103
                /*
                @lambdatest().noInit(latch).given(SEED, new LinkedList<>(Arrays.asList("foo", "bar"))).given(REPORTER, true).given(T_NUMBER, 0).given(SHA_512_DIGESTS, new ArrayList<>(Arrays.asList(new HMac(new SHA512Digest()))))
                        .given(SHA_256_DIGESTS, new ArrayList<>(Arrays.asList(MessageDigest.getInstance("SHA-256"))))
                        .given(WORKING_POSITION, 0).given(WORDS_TO_WORK, new ArrayList<>(Arrays.asList("a", "b"))).mock("processSeed(..)").mock("latch.countDown()").checkEq(SEED.get(1), "bar")
