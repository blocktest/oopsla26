...
public class StringInspector {
    ...
    public StringInspector(String source, int startingPosition, String openingMarkers, String closingMarkers, String overridingMarkers,
        ...
        String StringInspector3 = "Illegal argument value {0} for overridingMarkers. These cannot be null and must be a sub-set of openingMarkers {1}.";
        if (this.defaultSearchMode.contains(SearchMode.SKIP_BETWEEN_MARKERS)) {
            if (this.openingMarkers == null || this.closingMarkers == null || this.openingMarkers.length() != this.closingMarkers.length()) {
                String messages = "Illegal argument value {0} for openingMarkers and/or {1} for closingMarkers. These cannot be null and must have the same length.";
                throw new IllegalArgumentException(MessageFormat.format(messages, new String[] { this.openingMarkers, this.closingMarkers }));
            }
            if (this.overridingMarkers == null) {

                throw new IllegalArgumentException(MessageFormat.format(StringInspector3, new String[] { this.overridingMarkers, this.openingMarkers }));
            }
            ...
            if (checkSkipConditions && searchMode.contains(SearchMode.SKIP_BETWEEN_MARKERS) && this.openingMarkers.indexOf(c0) != -1) {
                ...
                if (dashDashCommentImmediateEnd) {
                    ...
                    if (this.pos >= this.stopAt) {
                        ...
                        // Reset lookahead.
                        // BLOCKTEST EVAL: https://github.com/pingcap/tidb-loadbalance/blob/b1d62a4561fee7c525ae4c640a10f55c4702755b/src/main/java/com/tidb/jdbc/conf/StringInspector.java#L418C24-L425C22
                        blocktest().given(c1, ' ').given(c0, '\r').given(this.pos, 5).given(c2, ' ').given(this.srcLen, 8).given(this.source, "Hello\rorld").checkEq(c1, 'o').checkEq(c2, 'r');
                        blocktest().given(c1, ' ').given(c0, '\r').given(this.pos, 5).given(c2, ' ').given(this.srcLen, 8).given(this.source, "Hello\r\nrld").checkEq(c1, 'r').checkEq(c2, Character.MIN_VALUE);
                        blocktest().given(c1, ' ').given(c0, '\r').given(this.pos, 5).given(c2, ' ').given(this.srcLen, 9).given(this.source, "Hello\r\nrld").checkEq(c1, 'r').checkEq(c2, 'l');
                        blocktest().given(c1, ' ').given(c0, 'W').given(this.pos, 5).given(c2, ' ').given(this.srcLen, 8).given(this.source, "HelloW\nrld").checkEq(c1, '\n').checkEq(c2, 'r');
                        blocktest().given(c1, ' ').given(c0, '\r').given(this.pos, 15).given(c2, ' ').given(this.srcLen, 8).given(this.source, "Hello\rorld").checkEq(c1, Character.MIN_VALUE).checkEq(c2, Character.MIN_VALUE);
                        blocktest().given(c1, ' ').given(c0, '\r').given(this.pos, 5).given(c2, ' ').given(this.srcLen, 7).given(this.source, "Hello\rorld").checkEq(c1, 'o').checkEq(c2, Character.MIN_VALUE);
                        c1 = this.pos + 1 < this.srcLen ? this.source.charAt(this.pos + 1) : Character.MIN_VALUE;
                        if (c0 == '\r' && c1 == '\n') {
                            // \r\n sequence found.
