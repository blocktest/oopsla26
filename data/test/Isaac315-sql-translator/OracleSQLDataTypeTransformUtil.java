    ...
    public static SQLDataType transformOracleToMySql(SQLDataType x) {
        ...
        if (nameHash == FnvHash.Constants.UROWID) {
            ...
            if (argumentns.size() > 0) {
                ...
                blocktest().given(argumentns, new ArrayList<>(Arrays.asList(new SQLNumberExpr(Double.parseDouble("12"))))).given(x, new SQLRecordDataType())
                        .checkEq("dataType", new SQLCharacterDataType("nvarchar", 12));
                blocktest().given(argumentns, new ArrayList<>(Arrays.asList(new SQLCharExpr("foo")))).given(x, new SQLRecordDataType())
