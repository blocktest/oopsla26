        ...
        for (RefererConfig c : refererList) {
            ...
            if (c.isReverse()) {
                if (!servers.containsKey(c.transport)) {
                    // BLOCKTEST EVAL: https://github.com/bruceran/krpc/blob/33c83d78574a472e3cbfc75afa87ec57253cc90c/src/main/java/krpc/rpc/bootstrap/Bootstrap.java#L598C1-L604C22
                    /*
                    @blocktest("test1").setup(() -> {
                        c = new RefererConfig("a");
                        c.setTransport("default");
                    }).given(lastServer, new ServerConfig("last")).given(servers, new LinkedHashMap<>()).checkTrue(servers.containsKey("default"));
                    */
                    /*
                    @blocktest("test2").setup(() -> {
                        c = new RefererConfig("a");
                        c.setTransport("default");
                        servers.put("foo", new ServerConfig("foo"));
                    }).given(lastServer, new ServerConfig("last")).given(servers, new LinkedHashMap<>()).checkTrue(c.transport.equals("last"));
                    */
                    /*
                    @blocktest("test3").setup(() -> {
                        c = new RefererConfig("a");
                        c.setTransport("foo");
                    }).given(lastServer, new ServerConfig("last")).given(servers, new LinkedHashMap<>()).expect(RuntimeException.class);
                     */
