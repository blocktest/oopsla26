    ...
    private ImmutableList<ImageConfiguration> getAllImages() {
        ...
        if (imagesMap != null) {
            imagesMap.forEach((alias, config) -> {
                ...
                blocktest().given(config, new ImageConfiguration()).given(alias, "foo").given(allImages, ImmutableList.builder())
                        .checkTrue(allImages.build().size() > 0);
                blocktest().given(config, new ImageConfiguration()).given(alias, "foo").given(allImages, ImmutableList.builder())
                        .checkEq(allImages.build().iterator().next().getAlias(), "foo");
                blocktest().given(config, new ImageConfiguration()).given(alias, "foo").setup(() -> {
                    config.setAlias("bar");
