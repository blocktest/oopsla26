                    ...
                    return MessageElement.builder(me.location())
                        ...
                        .fields(copyOf(me.fields().stream()
                            .map(f -> {
                                ...
                                /*
                                 @blocktest("line1").given(f, FieldElement.builder(com.squareup.wire.schema.Location.get("foo")).type("foo").tag(1).name("xxx").build())
                                   .given(renames, new HashMap<String, String>(){{put("foo", "bar");}}).checkEq(lambdaReturn.type(), "bar").checkEq(lambdaReturn.name(), "xxx");
                                */
                                /*
                                 @blocktest("line2").given(f, FieldElement.builder(com.squareup.wire.schema.Location.get("foo")).type("foo").tag(1).name("xxx").build())
