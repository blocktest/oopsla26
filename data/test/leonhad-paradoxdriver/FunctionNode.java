    ...
    public Set<FieldNode> getClauseFields() {
        if (fields == null) {
            fields = parameters.stream().map((SQLNode field) -> {
                ...
                blocktest().given(field, new FieldNode("foo", "bar", new ScannerPosition())).checkTrue(methodReturn instanceof FieldNode);
                blocktest().given(field, new NotNode( new ScannerPosition())).checkReturnEq(null);
