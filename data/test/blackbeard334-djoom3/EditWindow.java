...
//				memmove( &buffer[cursorPos], &buffer[cursorPos + 1], len - cursorPos);
                ...
                if (key == K_RIGHTARROW) {
                    if (cursorPos < len) {
                        ...
                        blocktest().given(buffer, new char[]{'a', 'b',  'c', ' ', ' ', 'd', 'e'}).setup(() -> {idKeyInput.Init(); idKeyInput.PreliminaryKeyEvent(K_CTRL, true);})
                            .given(cursorPos, 1).given(len, 7).checkEq(cursorPos, 5);
                        blocktest().given(buffer, new char[]{'a', 'b',  'c', ' ', ' ', 'd', 'e'}).setup(() -> {idKeyInput.Init(); idKeyInput.PreliminaryKeyEvent(K_CTRL, true);})
                            .given(cursorPos, 1).given(len, 2).checkEq(cursorPos, 2);
                        blocktest().given(buffer, new char[]{'a', 'b',  'c', ' ', ' ', 'd', 'e', 'f'}).setup(() -> {idKeyInput.Init(); idKeyInput.PreliminaryKeyEvent(K_CTRL, true);})
