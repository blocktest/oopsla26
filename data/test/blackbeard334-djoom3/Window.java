...
public class Window {
    ...
    public static class idWindow {
        ...
        // visible, focus, mouseover, cursor, border, etc..
        ...
        // overall rect
        protected idRectangle drawRect = new idRectangle();

        // client area
        protected idRectangle clientRect = new idRectangle();

        protected idVec2 origin = new idVec2();

        // 
        // time stamp used for various fx
        ...
        // 
        protected char /*signed*/
        textShadow;

        protected char /*unsigned*/
        fontNum;
        ...
        // overall rect
        protected idWinRectangle rect = new idWinRectangle();
        ...
        // 
        // text extented rect
        protected idRectangle textRect = new idRectangle();
        ...
        // 
        // parent window
        protected idWindow parent;

        // child windows
        protected idList<idWindow> children = new idList<>();

        protected idList<drawWin_t> drawWindows = new idList<>();

        // 
        // if a child window has the focus
        protected idWindow focusedChild;

        // if a child window has mouse capture
        protected idWindow captureChild;

        // if a child window has mouse capture
        protected idWindow overChild;
        ...
        // 
        // statics to assist during parsing
        protected static boolean[] registerIsTemporary = new boolean[MAX_EXPRESSION_REGISTERS];

        // 
        // evaluate to make expressionRegisters
        protected idList<wexpOp_t> ops = new idList<>();

        protected idList<Float> expressionRegisters = new idList<>();

        // evaluate to make expressionRegisters
        protected idList<wexpOp_t>[] saveOps;

        // added named events
        protected idList<rvNamedEvent> namedEvents = new idList<>();
        ...
        // 
        static idMat3 trans = new idMat3();

        static idVec3 org = new idVec3();

        static idRotation rot = new idRotation();

        static final idVec3 vec = new idVec3(0, 0, 1);

        static idMat3 smat = new idMat3();
        ...
        public static final String[] ScriptNames = { "onMouseEnter", "onMouseExit", "onAction", "onActivate", "onDeactivate", "onESC", "onEvent", "onTrigger", "onActionRelease", "onEnter", "onEnterRelease" };

        public static final idRegEntry[] RegisterVars = { new idRegEntry("forecolor", VEC4), new idRegEntry("hovercolor", VEC4), new idRegEntry("backcolor", VEC4), new idRegEntry("bordercolor", VEC4), new idRegEntry("rect", RECTANGLE), new idRegEntry("matcolor", VEC4), new idRegEntry("scale", VEC2), new idRegEntry("translate", VEC2), new idRegEntry("rotate", FLOAT), new idRegEntry("textscale", FLOAT), new idRegEntry("visible", BOOL), new idRegEntry("noevents", BOOL), new idRegEntry("text", STRING), new idRegEntry("background", STRING), new idRegEntry("runscript", STRING), new idRegEntry("varbackground", STRING), new idRegEntry("cvar", STRING), new idRegEntry("choices", STRING), new idRegEntry("choiceVar", STRING), new idRegEntry("bind", STRING), new idRegEntry("modelRotate", VEC4), new idRegEntry("modelOrigin", VEC4), new idRegEntry("lightOrigin", VEC4), new idRegEntry("lightColor", VEC4), new idRegEntry("viewOffset", VEC4), new idRegEntry("hideCursor", BOOL) };
        ...
        public void SetFlag(/*unsigned*/
        int f) {
            flags |= f;
        }

        public void ClearFlag(/*unsigned*/
        int f) {
            flags &= ~f;
        }
        ...
        static int DBG_SetupBackground = 0;
        ...
        public void SetupBackground() {
            ...
            if (backGroundName.Length() != 0) {
                ...
                background.SetImageClassifications(1);
        ...
        static final drawWin_t dw = new drawWin_t();
        ...
        public idWinVar GetWinVarByName(final String _name, boolean fixup, /*= false*/
        ...
        public boolean Parse(idParser src, boolean rebuild) /*= true*/
        {
            ...
            while (!token.equals("}")) {
                ...
                if (token.equals("onNamedEvent")) {
                    ...
                    src.SetMarker();
                    if (!ParseScript(src, ev.event, null)) {
                        ret = false;
                        break;
                    }
        ...
        static boolean actionDownRun;

        static boolean actionUpRun;
            ...
            if (visible.data && !noEvents.data) {
                if (event.evType == SE_KEY) {
                    ...
                    if (event.evValue == K_MOUSE1) {
                        ...
                        int c = children.Num();
                        while (--c >= 0) {
                            if (children.oGet(c).visible.data && children.oGet(c).Contains(children.oGet(c).drawRect, gui.CursorX(), gui.CursorY()) && !(children.oGet(c).noEvents.data)) {
        ...
        public void Redraw(float x, float y) {
            ...
            drawCursorTotal++;
            if (gui_edit.GetBool() || ((flags & WIN_DESKTOP) != 0 && 0 == (flags & WIN_NOCURSOR) && !hideCursor.data && (gui.Active() || ((flags & WIN_MENUGUI) != 0)))) {
        ...
        static float[] regs = new float[MAX_EXPRESSION_REGISTERS];

        static idWindow lastEval;
        ...
        public boolean UpdateFromDictionary(idDict dict) {
            ...
            for (i = 0; i < dict.GetNumKeyVals(); i++) {
                ...
                idParser src = new idParser(kv.GetValue().toString(), kv.GetValue().Length(), "", LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT | LEXFL_ALLOWMULTICHARLITERALS | LEXFL_ALLOWBACKSLASHSTRINGCONCAT);
        ...
        protected void Transition() {
            ...
            for (i = 0; i < c; i++) {
                ...
                } else if (data.data instanceof idWinFloat) {
                    // TODO:check empty cast(s)(below too I think). EDIT:casts are to check types.
        ...
        /*
         ...
         =================
         */
        protected int ParseTerm(idParser src, idWinVar var, /*= NULL*/
        int component) /*= 0*/
        {
            idToken token = new idToken();
            idWinVar a;
            ...
            if (var != null) {
                ...
                } else if (var instanceof idWinFloat) {
                    // dynamic_cast < idWinFloat > (var)) {
                    return EmitOp(a, b, WOP_TYPE_VARF);
                } else if (var instanceof idWinInt) {
                    // (dynamic_cast < idWinInt > (var)) {
                    return EmitOp(a, b, WOP_TYPE_VARI);
                } else if (var instanceof idWinBool) {
                    // (dynamic_cast < idWinBool > (var)) {
                    return EmitOp(a, b, WOP_TYPE_VARB);
                } else if (var instanceof idWinStr) {
                    // (dynamic_cast < idWinStr > (var)) {
        ...
        /*
         ...
         ===============
         */
        static int DBG_EvaluateRegisters = 0;
        ...
        static int DBG_ParseRegEntry = 0;
