...
@Command(name = "heroku-jvm-application-deployer", mixinStandardHelpOptions = true, description = "Application for deploying Java applications to Heroku.", defaultValueProvider = DefaultValueProvider.class)
public class Main implements Callable<Integer> {
    ...
    public Integer call() throws Exception {
        ...
        if (!disableAutoIncludes) {
            List<Path> defaultPaths = Arrays.asList(Paths.get("Procfile"), Paths.get("system.properties"), Paths.get(".jdk-overlay"));
        ...
        if (jdkString.isPresent()) {
            ...
            if (sourceBlobDescriptor.containsPath(systemPropertiesPath)) {
                ...
                sourceBlobDescriptor.addSyntheticFile("system.properties", String.format("java.runtime.version=%s", jdkString.get()), true);
            }
        }
        ...
        DeploymentDescriptor deploymentDescriptor = new DeploymentDescriptor(appName.get(), buildpacks, Collections.emptyMap(), sourceBlobArchive, GitUtils.getHeadCommitHash(projectDirectory).orElse("nogit-" + sourceBlobArchiveSha256));
        String version = PropertiesUtils.loadPomPropertiesOrEmptyFromClasspath(Main.class, "com.heroku", "heroku-jvm-application-deployer").getProperty("version", "unknown");
        boolean deploySuccessful = Deployer.deploy(apiKey.get(), "heroku-jvm-application-deployer", version, deploymentDescriptor);
        if (deploySuccessful) {
            return 0;
        } else {
            return -1;
    ...
    private Optional<Procfile> generateProcfile() {
        ...
        return PathUtils.getFileExtension(mainFile.get()).flatMap(extension -> {
            ...
            String normalizedMainFile = PathUtils.normalize(projectDirectory, mainFile.get()).map(PathUtils::separatorsToUnix).orElse("");
            if (extension.equalsIgnoreCase("jar")) {
                return Optional.of(Procfile.singleton("web", String.format("java $JAVA_OPTS -jar %s %s $JAR_OPTS", normalizedMainFile, jarFileOpts.orElse(""))));
            } else if (extension.equalsIgnoreCase("war")) {
                return Optional.of(Procfile.singleton("web", String.format("java $JAVA_OPTS -jar %s $WEBAPP_RUNNER_OPTS --port $PORT %s", WEBAPP_RUNNER_SOURCE_BLOB_PATH, normalizedMainFile)));
            }
            return Optional.empty();
        });
    }
    ...
    static final Path WEBAPP_RUNNER_SOURCE_BLOB_PATH = Paths.get(".heroku/webapp-runner.jar");
}
