...
import static com.pawelgorny.lostword.WorkerPrintSeeds.*;

public class WorkerPrintSeedsBlockTest {

    @Test
    public void testLine1() throws Exception {
        CountDownLatch latch = null;
        List<String> SEED = new ArrayList<>(java.util.Arrays.asList("foo", "bar"));
        boolean REPORTER = true;
        int T_NUMBER = 0;
        List<HMac> SHA_512_DIGESTS = new ArrayList<>(java.util.Arrays.asList(new HMac(new org.bouncycastle.crypto.digests.SHA512Digest())));
        List<MessageDigest> SHA_256_DIGESTS = new ArrayList<>(java.util.Arrays.asList(MessageDigest.getInstance("SHA-256")));
        int WORKING_POSITION = 0;
        List<String> WORDS_TO_WORK = new ArrayList<>(java.util.Arrays.asList("a", "b"));
        long start = 0;
        if (REPORTER) {
