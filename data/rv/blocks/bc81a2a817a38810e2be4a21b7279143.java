    ...
    private static DecryptResult decryptPKCS8(ASN1Structure pkcs8,
        ...
        if (isVersion2) {
            ...
            // 2.16.840.1.101.3.4.1.44 - id-aes256-CFB
                ...
                // BLOCKTEST EVAL: https://github.com/narupley/not-going-to-be-commons-ssl/blob/346bb4e8481316687a643219800a398ecd8f62a8/src/main/java/org/apache/commons/ssl/PKCS8Key.java#L659C1-L695C18
                blocktest().given(oid, "2.16.840.1.101.3.4.1.1").checkEq(mode, "ECB").given(keySize, 128);
                blocktest().given(oid, "2.16.840.1.101.3.4.1.2.1").checkEq(mode, "CBC").given(keySize, 128);
                blocktest().given(oid, "2.16.840.1.101.3.4.1.23").checkEq(mode, "OFB").given(keySize, 192);
                blocktest().given(oid, "2.16.840.1.101.3.4.1.44").checkEq(mode, "CFB").given(keySize, 256);
                blocktest().given(oid, "2.16.840.1.101.3.4.1.25").expect(RuntimeException.class);
                blocktest().given(oid, "2.16.840.1.101.3.4.1.14").expect(RuntimeException.class);
