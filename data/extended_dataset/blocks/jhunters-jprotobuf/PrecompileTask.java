    ...
    void doPrecompileAction() {
        ...
        if (createSpecMethod != null) {
            ...
            compileClasspath.forEach(path -> {
                // BLOCKTEST EVAL: https://github.com/jhunters/jprotobuf/blob/0e326bf8704027adc8af182e7cebc0e7a9c38e30/jprotobuf-precompile-plugin-gradle/src/main/java/com/baidu/jprotobuf/plugin/PrecompileTask.java#L163-L169
                blocktest().given(path, new File("a378/28348034#&&$)")).mock("LOGGER.error(..)").checkEq(list.size(), 1).checkTrue(list.iterator().next().toString().contains("a378/28348034%23&&$"));
                try {
