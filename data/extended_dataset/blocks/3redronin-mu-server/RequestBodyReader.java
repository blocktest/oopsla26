        ...
        try {
            ...
            if (soFar > maxSize) {
                ...
                onRequestBodyRead0(content, last, error -> {
                    ...
                    }).given(error, null, "Throwable").given(last, false).checkFlow(IfStmt().ElseIf());
                    blocktest().given(callback, new DoneCallback() {
                        @Override
                        public void onComplete(Throwable error) throws Exception {

                        }
                    }).given(error, null, "Throwable").given(last, true).checkFlow(IfStmt().ElseIf().Then());
                    blocktest().given(callback, new DoneCallback() {
                        @Override
                        public void onComplete(Throwable error) throws Exception {

                        }
                    }).given(error, new FileNotFoundException(), "Throwable").given(last, true).checkFlow(IfStmt().Then());
                    if (error != null) {
            ...
            try {
                if (content.readableBytes() > 0) {
                    DoneCallback successCalled = !last ? callback : error -> {
                        ...
                        }).given(error, null, "Throwable").checkFlow(IfStmt().Then());
                        blocktest().given(readListener, new RequestBodyListener() {
                            @Override
                            public void onDataReceived(ByteBuffer buffer, DoneCallback doneCallback) throws Exception {
                        ...
                        }).given(error, new FileNotFoundException(), "Throwable").checkFlow(IfStmt().Else());
                        if (error == null) {
