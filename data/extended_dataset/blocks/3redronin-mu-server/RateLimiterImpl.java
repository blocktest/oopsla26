    ...
    boolean record(MuRequest request) {
        ...
        if (curVal >= rateLimit.allowed) {
            ...
            timer.newTimeout(timeout -> {
                ...
                blocktest().given(counter, new AtomicLong(-3)).given(name, "foo").setup(() -> {
                    map.put("hey", new AtomicLong(1));
                    map.put("foo", new AtomicLong(12));
                }).checkFalse(map.containsKey("foo")).checkEq(map.get("hey").toString(), "1");
                blocktest().given(counter, new AtomicLong(1)).given(name, "foo").setup(() -> {
                    map.put("hey", new AtomicLong(1));
                    map.put("foo", new AtomicLong(12));
                }).checkFalse(map.containsKey("foo")).checkEq(map.get("hey").toString(), "1");
                blocktest().given(counter, new AtomicLong(0)).given(name, "foo").setup(() -> {
                    map.put("hey", new AtomicLong(1));
                    map.put("foo", new AtomicLong(12));
                }).checkFalse(map.containsKey("foo")).checkEq(map.get("hey").toString(), "1");
                blocktest().given(counter, new AtomicLong(3)).given(name, "foo").setup(() -> {
