    ...
    private CompletionStage<?> write(String text) {
        ...
        if (closed) {
            ...
            asyncHandle.write(Mutils.toByteBuffer(text), error -> {
                ...
                blocktest().given(error, new FileNotFoundException(), "Throwable").given(stage, new CompletableFuture<String>(), "CompletableFuture<String>").checkTrue(stage.isCompletedExceptionally());
                blocktest().given(error, null, "Throwable").given(stage, new CompletableFuture<String>(), "CompletableFuture<String>").checkFalse(stage.isCompletedExceptionally());
                if (error == null) {
                    stage.complete(null);
                } else {
