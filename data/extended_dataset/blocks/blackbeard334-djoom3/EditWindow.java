...
//				memmove( &buffer[cursorPos], &buffer[cursorPos + 1], len - cursorPos);
                ...
                if (key == K_RIGHTARROW) {
                    if (cursorPos < len) {
                        // BLOCKTEST EVAL: https://github.com/blackbeard334/djoom3/blob/64362645420c99ac621b051a678de96883e2a329/src/main/java/neo/ui/EditWindow.java#L287C1-L300C26
             
                        blocktest().given(buffer, new char[]{'a', 'b',  'c', ' ', ' ', 'd', 'e'}, "char[]").setup(() -> {idKeyInput.Init(); idKeyInput.PreliminaryKeyEvent(K_CTRL, true);})
                            .given(cursorPos, 1).given(len, 7).checkEq(cursorPos, 5);
                        blocktest().given(buffer, new char[]{'a', 'b',  'c', ' ', ' ', 'd', 'e'}, "char[]").setup(() -> {idKeyInput.Init(); idKeyInput.PreliminaryKeyEvent(K_CTRL, true);})
                            .given(cursorPos, 1).given(len, 2).checkEq(cursorPos, 2);
                        blocktest().given(buffer, new char[]{'a', 'b',  'c', ' ', ' ', 'd', 'e', 'f'}, "char[]").setup(() -> {idKeyInput.Init(); idKeyInput.PreliminaryKeyEvent(K_CTRL, true);})
                            .given(cursorPos, 1).given(len, 6).checkEq(cursorPos, 5);
                         blocktest().given(buffer, new char[]{'a', 'b',  'c', ' ', ' ', 'd', 'e', 'f'}, "char[]").setup(() -> {idKeyInput.Init(); idKeyInput.PreliminaryKeyEvent(K_CTRL, true);})
                            .given(cursorPos, 1).given(len, 4).checkEq(cursorPos, 4);
