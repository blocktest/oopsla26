...
//                    }
        ...
        private boolean CanSwimInArea(int areaNum) {
            return (file.areas.oGet(areaNum).contents & AREACONTENTS_WATER) != 0;
        }

        private boolean AreaHasFloor(int areaNum) {
            return (file.areas.oGet(areaNum).flags & AREA_FLOOR) != 0;
        }

        private boolean AreaIsClusterPortal(int areaNum) {
            return (file.areas.oGet(areaNum).flags & AREACONTENTS_CLUSTERPORTAL) != 0;
        }
        ...
        private void Reachability_Fly(int areaNum) {
            ...
            for (i = 0; i < area.numFaces; i++) {
                ...
                face = file.faces.oGet(abs(faceNum));
                ...
                for (k = 0; k < area.numFaces; k++) {
                    ...
                    for (l = 0; l < otherArea.numFaces; l++) {
                        ...
                        for (m = 0; m < face1.numEdges; m++) {
                            edge1Num = abs(file.edgeIndex.oGet(face1.firstEdge + m));
                            for (n = 0; n < face2.numEdges; n++) {
                                edge2Num = abs(file.edgeIndex.oGet(face2.firstEdge + n));
        ...
        private boolean Reachability_Step_Barrier_WaterJump_WalkOffLedge(int fromAreaNum, int toAreaNum) {
            ...
            for (i = 0; i < area1.numFaces; i++) {
                ...
                floorFace1 = file.faces.oGet(abs(floorFace1Num));
                ...
                for (k = 0; k < floorFace1.numEdges; k++) {
                    ...
                    for (j = 0; j < area2.numFaces; j++) {
                        floorFace2 = file.faces.oGet(abs(file.faceIndex.oGet(area2.firstFace + j)));
                        ...
                        for (l = 0; l < floorFace2.numEdges; l++) {
                            ...
                            if ((x1 - 0.5f < x3 && x4 < x2 + 0.5f) && (x3 - 0.5f < x1 && x2 < x4 + 0.5f)) {
                                ...
                                if (x1 > x3 - 0.1f && x1 < x3 + 0.1f) {
                                    ...
                                    p1area1.oSet(2, y);
                                    p1area2 = v3;
                                    // BLOCKTEST EVAL: https://github.com/blackbeard334/djoom3/blob/64362645420c99ac621b051a678de96883e2a329/src/main/java/neo/Tools/Compilers/AAS/AASReach.java#L542C1-L546C50
                                    blocktest().given(y1, 1).given(x3, 3).given(x1, 1).given(y2, 2).given(x2, 2)
                                            .given(v3, new idVec3()).given(y3, 3).given(p1area1, new idVec3()).given(p1area2, new idVec3()).given(dist1, 0)
                                            .checkEq(dist1, 0.0, Double.MIN_VALUE).start(FIRST_BLOCK);
                                } else {
                            ...
                            // BLOCKTEST EVAL: https://github.com/blackbeard334/djoom3/blob/64362645420c99ac621b051a678de96883e2a329/src/main/java/neo/Tools/Compilers/AAS/AASReach.java#L575C25-L587C30
                            // Case if
                            blocktest().given(dist1, 3).given(dist2, 3).given(p1area1, new idVec3()).given(p1area2, new idVec3())
                                    .given(p2area1, new idVec3()).given(p2area2, new idVec3()).given(dist, 0)
                                    .checkEq(dist, 3.0f, Float.MIN_VALUE).checkEq(start, new idVec3()).checkEq(end, new idVec3())
                                    .start(FIRST_BLOCK);
                            // Case elif
                            blocktest().given(dist1, 1).given(dist2, 2).given(p1area1, new idVec3()).given(p2area1, new idVec3())
                                    .given(p1area2, new idVec3()).given(p2area2, new idVec3()).given(dist, 0)
                                    .given(start, new idVec3()).given(end, new idVec3())
                                    .checkEq(dist, 1.0f, Float.MIN_VALUE).checkEq(start, new idVec3()).checkEq(end, new idVec3())
                                    .start(FIRST_BLOCK);
                            // Case else
                            blocktest().given(dist1, 5).given(dist2, 4).given(p1area1, new idVec3()).given(p2area1, new idVec3())
                                    .given(p1area2, new idVec3()).given(p2area2, new idVec3()).given(dist, 0)
                                    .given(start, new idVec3()).given(end, new idVec3())
                                    .checkEq(dist, 4.0f, Float.MIN_VALUE).checkEq(start, new idVec3()).checkEq(end, new idVec3())
                                    .start(FIRST_BLOCK);
        ...
        private void Reachability_WalkOffLedge(int areaNum) {
            ...
            for (i = 0; i < area.numFaces; i++) {
                ...
                for (j = 0; j < face.numEdges; j++) {
                    ...
                    edge = file.edges.oGet(abs(edgeNum));
