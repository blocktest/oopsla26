    ...
    public static void merge(List<String> sources, String target) throws IOException{
        ...
        for(String source : sources){
            try(Stream<String> lines = Files.lines(Paths.get(source), Charset.forName("utf-8"))){
                lines.forEach(line -> {
                    // BLOCKTEST EVAL: https://github.com/ysc/word/blob/daa57fa34645e39c6e0214d1f4698134963b65be/src/main/java/org/apdplat/word/dictionary/DictionaryTools.java#L100-L114
                    blocktest().given(i, new AtomicInteger(0)).given(line, "a   ").given(set, new HashSet<String>()).checkTrue(set.size() == 0);
                    blocktest().given(i, new AtomicInteger(0)).given(line, "   abcde").given(set, new HashSet<String>()).checkTrue(set.size() == 0);
                    blocktest().given(i, new AtomicInteger(0)).given(line, " 词长 ").given(set, new HashSet<String>()).checkTrue(set.iterator().next().equals("词长"));
                    blocktest().given(i, new AtomicInteger(0)).given(line, "没没没").given(set, new HashSet<String>()).checkTrue(set.iterator().next().equals("没没没"));
                    blocktest().given(i, new AtomicInteger(0)).given(line, " 一一 ").given(set, new HashSet<String>()).checkTrue(set.size() == 0);
                    blocktest().given(i, new AtomicInteger(0)).given(line, "a一一").given(set, new HashSet<String>()).checkTrue(set.size() == 0);
