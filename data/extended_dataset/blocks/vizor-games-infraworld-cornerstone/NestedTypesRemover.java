                ...
                if (t instanceof MessageElement)
                {
                    ...
                    return MessageElement.builder(me.location())
                        ...
                        .fields(copyOf(me.fields().stream()
                            .map(f -> {
                                // BLOCKTEST EVAL: https://github.com/vizor-games/infraworld-cornerstone/blob/2f8c792393fd8bbfc09d5bfdd36114230f64aa5c/src/main/java/com/vizor/unreal/preprocess/NestedTypesRemover.java#L116-L129
                                /*
                                 @blocktest("line1").given(f, FieldElement.builder(com.squareup.wire.schema.Location.get("foo")).type("foo").tag(1).name("xxx").build(), "FieldElement")
                                   .given(renames, new HashMap<String, String>(){{put("foo", "bar");}}).checkEq(lambdaReturn.type(), "bar").checkEq(lambdaReturn.name(), "xxx");
                                */
                                /*
                                 @blocktest("line2").given(f, FieldElement.builder(com.squareup.wire.schema.Location.get("foo")).type("foo").tag(1).name("xxx").build(), "FieldElement")
                                   .given(renames, new HashMap<String, String>(){{put("foo2", "bar");}}).checkEq(lambdaReturn.type(), "foo").checkEq(lambdaReturn.name(), "xxx");
                                */
