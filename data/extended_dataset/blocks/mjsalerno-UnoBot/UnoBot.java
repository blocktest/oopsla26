...
public class UnoBot extends ListenerAdapter {
    ...
    static final String UR_TURN = " it is your turn.";

    static final String ERR_SAVE_MSG = "Sorry but I could not save the message ";

    static final String MSG_FILE_NAME = "Messages.dat";

    static final String TOP_CARD = "Top Card: ";
    ...
    public void onMessage(MessageEvent event) throws Exception {
        ...
        if (!message.startsWith(this.token)) {
            // if not message starts with token, its not a command for this game
            return;
        }
        // replace n amount of spaces with only one
        ...
        if (tokens[0].equalsIgnoreCase(this.token + "uno")) {
            if (gameUp) {
                ...
                // BLOCKTEST EVAL: https://github.com/mjsalerno/UnoBot/blob/4ee2ef9abe11a299a434f9ea998b6dddd8a4b0aa/src/main/java/com/mjsalerno/unobot/UnoBot.java#L576C1-L587C18
                blocktest().given(tokens, new String[] {}).given(this.extreme, false).given(this.attack, false).checkFalse(this.extreme).checkFalse(this.attack);
                blocktest().given(tokens, new String[] { "hi" }).given(this.extreme, false).given(this.attack, false).checkFalse(this.extreme).checkFalse(this.attack);
                blocktest().given(tokens, new String[] { "hi", "+E" }).given(this.extreme, false).given(this.attack, false).checkTrue(this.extreme).checkFalse(this.attack);
                blocktest().given(tokens, new String[] { "hi", "+Ex" }).given(this.extreme, false).given(this.attack, false).checkFalse(this.extreme).checkFalse(this.attack);
                blocktest().given(tokens, new String[] { "hi", "+e" }).given(this.extreme, false).given(this.attack, false).checkTrue(this.extreme).checkFalse(this.attack);
                blocktest().given(tokens, new String[] { "hi", "", "+e" }).given(this.extreme, false).given(this.attack, false).checkTrue(this.extreme).checkFalse(this.attack);
                blocktest().given(tokens, new String[] { "hi", "", "+E" }).given(this.extreme, false).given(this.attack, false).checkTrue(this.extreme).checkFalse(this.attack);
                blocktest().given(tokens, new String[] { "hi", "+A" }).given(this.extreme, false).given(this.attack, false).checkFalse(this.extreme).checkTrue(this.attack);
                blocktest().given(tokens, new String[] { "hi", "+Ax" }).given(this.extreme, false).given(this.attack, false).checkFalse(this.extreme).checkFalse(this.attack);
                blocktest().given(tokens, new String[] { "hi", "+a" }).given(this.extreme, false).given(this.attack, false).checkFalse(this.extreme).checkTrue(this.attack);
                blocktest().given(tokens, new String[] { "hi", "", "+a" }).given(this.extreme, false).given(this.attack, false).checkFalse(this.extreme).checkTrue(this.attack);
                blocktest().given(tokens, new String[] { "hi", "", "+A" }).given(this.extreme, false).given(this.attack, false).checkFalse(this.extreme).checkTrue(this.attack);
                blocktest().given(tokens, new String[] { "hi", "+e", "+A" }).given(this.extreme, false).given(this.attack, false).checkTrue(this.extreme).checkTrue(this.attack);
        ...
        if ((tokens[0].equalsIgnoreCase(this.token + "play") || tokens[0].equalsIgnoreCase(this.token + "p") || tokens[0].equalsIgnoreCase(this.token + "pl")) && delt && gameUp && (sender.equals(players.at().getName()))) {
            ...
            if (tokens.length >= 2) {
                if (tokens.length >= 3) {
                    ...
                    // allow short variants with out space: Y2, G3 etc
    ...
    private void dealCards() {
        stopUnoTimer();
        // new game starting so no one has drawn
        drew = false;
