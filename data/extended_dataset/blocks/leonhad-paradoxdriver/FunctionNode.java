    ...
    public Set<FieldNode> getClauseFields() {
        if (fields == null) {
            fields = parameters.stream().map((SQLNode field) -> {
                // BLOCKTEST EVAL: https://github.com/leonhad/paradoxdriver/blob/de3a6ad18b6c7f1d461383876339dc8086beaead/src/main/java/com/googlecode/paradox/planner/nodes/FunctionNode.java#L104-L110
                blocktest().given(field, new FieldNode("foo", "bar", new ScannerPosition()), "SQLNode").checkTrue(methodReturn instanceof FieldNode);
                blocktest().given(field, new NotNode( new ScannerPosition()), "SQLNode").checkReturnEq(null);
