		...
		if (sort != null) {
			ignoreCaseOrderList = StreamUtils.createStreamFromIterator(sort.iterator())
					.map(order -> {
						...
						blocktest().given(caseSensitiveSortProperties, new ArrayList<>()).given(order, new Sort.Order("foo"), "Sort.Order").checkEq(methodReturn.toString(), "foo: ASC");
						blocktest().given(caseSensitiveSortProperties, new ArrayList<>(Arrays.asList("hi", "bye"))).given(order, new Sort.Order("bye"), "Sort.Order").checkEq(methodReturn.toString(), "bye: ASC, ignoring case");
