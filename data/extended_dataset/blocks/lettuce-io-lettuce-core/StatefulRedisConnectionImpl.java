        ...
        if (command.getType().name().equals(MULTI.name())) {
            ...
            if (command instanceof CompleteableCommand) {
                ((CompleteableCommand<?>) command).onComplete((ignored, e) -> {
                    // BLOCKTEST EVAL: https://github.com/redis/lettuce/blob/761d60247159662ce1db8bc20318bc4b573bc010/src/main/java/io/lettuce/core/StatefulRedisConnectionImpl.java#L188-L192
    ...
    protected <T> RedisCommand<K, V, T> preProcessCommand(RedisCommand<K, V, T> command) {
        ...
        if (local.getType().name().equals(SELECT.name())) {
            local = attachOnComplete(local, status -> {
                ...
                blocktest().given(state,  new ConnectionState()).given(status, "OK", "String").given(command, new Command<>(CommandType.GET, null, new CommandArgs<>(StringCodec.UTF8).add("SLEEP").add(1)), "RedisCommand<String, String, String>").checkEq(state.getDb(), 1);
                blocktest().given(state,  new ConnectionState()).given(status, "OK", "String").given(command, new Command<>(CommandType.GET, null, new CommandArgs<>(StringCodec.UTF8).add("SLEEP").add(10)), "RedisCommand<String, String, String>").checkEq(state.getDb(), 10);
                blocktest().given(state,  new ConnectionState()).given(status, "OK", "String").given(command, new Command<>(CommandType.GET, null, new CommandArgs<>(StringCodec.UTF8).add("SLEEP")), "RedisCommand<String, String, String>").checkEq(state.getDb(), 0);
                blocktest().given(state,  new ConnectionState()).given(status, "NOTOK", "String").given(command, new Command<>(CommandType.GET, null, new CommandArgs<>(StringCodec.UTF8).add("SLEEP").add(1)), "RedisCommand<String, String, String>").checkEq(state.getDb(), 0);
                if ("OK".equals(status)) {
