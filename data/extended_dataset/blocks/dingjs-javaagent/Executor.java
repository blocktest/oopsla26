...
public class Executor implements Opcode {
    ...
    public void execute(MethodInfo method, int pos, CodeIterator iter, Frame frame, Subroutine subroutine) throws BadBytecode {
        ...
        switch (opcode) {
            ...
            case DUP2_X1:
            case DUP2_X2: {
                // BLOCKTEST EVAL: https://github.com/dingjs/javaagent/blob/aa1cea60d0042f58977fb5d42c16afca92ad589d/src/main/java/com/wenshuo/agent/javassist/bytecode/analysis/Executor.java#L282C1-L293C51
                blocktest().given(frame, new Frame(1000, 1000)).given(DUP2_X1, 93, "int").given(opcode, 0)
                        .setup(() -> {
                            for (int i = 0; i < 500; i++) {
                                frame.push(Type.DOUBLE);
                            }
                        }).checkEq(frame.pop(), Type.DOUBLE);
                int end = frame.getTopIndex();
