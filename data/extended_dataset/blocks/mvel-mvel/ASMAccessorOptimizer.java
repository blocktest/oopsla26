    ...
    if (member instanceof Field) {
      ...
      try {
        ...
        if (member.getDeclaringClass().equals(ctx)) {
            ...
            // Case fallthru
            // @blocktest("fallthru").given(member, ASMAccessorOptimizer.class.getMethod("isLiteralOnly")).given(property, "isLiteralOnly").given(expr, new char[]{}, "char[]").given(tkStart, 0, "int").expect(null);
            // Case NoSuchMethodException
            // @blocktest("NSM").given(member, ASMAccessorOptimizer.class.getMethod("isLiteralOnlyX")).given(property, "isLiteralOnlyX").given(expr, new char[]{}, "char[]").given(tkStart, 0, "int").expect(NoSuchMethodException.class);
            try {
