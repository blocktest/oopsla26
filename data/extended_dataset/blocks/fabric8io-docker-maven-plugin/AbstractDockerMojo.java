    ...
    private ImmutableList<ImageConfiguration> getAllImages() {
        ...
        if (imagesMap != null) {
            imagesMap.forEach((alias, config) -> {
                // BLOCKTEST EVAL: https://github.com/fabric8io/docker-maven-plugin/blob/40371e754a6f3122af6d3019508c364627580935/src/main/java/io/fabric8/maven/docker/AbstractDockerMojo.java#L495-L500
                blocktest().given(config, new ImageConfiguration(), "ImageConfiguration").given(alias, "foo", "String").given(allImages, ImmutableList.builder())
                        .checkTrue(allImages.build().size() > 0);
                blocktest().given(config, new ImageConfiguration(), "ImageConfiguration").given(alias, "foo", "String").given(allImages, ImmutableList.builder())
                        .checkEq(allImages.build().iterator().next().getAlias(), "foo");
                blocktest().given(config, new ImageConfiguration(), "ImageConfiguration").given(alias, "foo", "String").setup(() -> {
                    config.setAlias("bar");
                        }).given(allImages, ImmutableList.builder())
                        .checkEq(allImages.build().iterator().next().getAlias(), "bar");
