    ...
    public SQL from(SQL... cc) {
        for (SQL c : cc) {
            c.getSqlPiepline().getSqlNexts().forEach(sqlNext -> {
                // BLOCKTEST EVAL: https://github.com/SpringStudent/GyJdbc/blob/0ca78eb623487f137d94eb719a94464d2d0cbbe8/src/main/java/com/gysoft/jdbc/bean/SQL.java#L107-L115
                blocktest().given(sqlNext, new SQLPiepline.SQLNext(new SQL("a"), "U"), "SQLPiepline.SQLNext")
                        .given(subSqls, new ArrayList<>())
                        .checkTrue(subSqls.size() == 1).checkEq(subSqls.iterator().next().getUnionType(), "U");
                blocktest().given(sqlNext, new SQLPiepline.SQLNext(new SQL("a"), null), "SQLPiepline.SQLNext")
                        .given(subSqls, new ArrayList<>())
                        .checkTrue(subSqls.size() == 1).checkEq(subSqls.iterator().next().getUnionType(), ",");
