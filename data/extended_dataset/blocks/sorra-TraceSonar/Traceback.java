  ...
  public Traceback(boolean isHtml, boolean includePotentialCalls, boolean onlySearchDirectCalls) {
    ...
    if (isHtml) {
      ...
      printer = node -> {
        ...
        blocktest().given(output, new StringBuilder()).given(node, new TreeNode(new sorra.tracesonar.model.Method("a", "b", "c"), false, null), "TreeNode").checkEq(output.toString(), "<- a #b c\n");
        blocktest().given(output, new StringBuilder()).given(node, new TreeNode(new sorra.tracesonar.model.Method("a", "b", "c"), false, null), "TreeNode").setup(() -> {node.setError("error");}).checkEq(output.toString(), "<- a #b c {error}\n");
