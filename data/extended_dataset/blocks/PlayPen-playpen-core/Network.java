    ...
    public boolean receive(Protocol.AuthenticatedMessage auth, Channel from) {
        ...
        if (local.getChannel() == from) {
            from.closeFuture().addListener(channelFuture -> {
                // BLOCKTEST EVAL: https://github.com/PlayPen/playpen-core/blob/dd867086481d93a439cb0270eaa7272bacf75493/src/main/java/io/playpen/core/coordinator/network/Network.java#L375-L384
                blocktest().given(consoles, new HashMap<>()).given(local, new LocalCoordinator())
                    .setup(() -> {
                        local.setUuid("123");
                    ...
                    }).mock("sendDetachConsole(..)").checkEq(consoles.size(), 1).end(FIRST_BLOCK, 10);
                ...
                blocktest().given(consoles, new HashMap<>()).given(local, new LocalCoordinator())
                  .setup(() -> {
                    ...
                    c2.setCoordinator("coord2");
                    consoles.put("console2", c2);
                }).mock("sendDetachConsole(..)").checkEq(consoles.size(), 0).end(FIRST_BLOCK, 10);
                ...
                blocktest().given(consoles, new HashMap<>()).given(local, new LocalCoordinator())
                  .setup(() -> {
                    ...
                    c2.setCoordinator("coord2");
                    consoles.put("console2", c2);
                }).mock("sendDetachConsole(..)").checkEq(consoles.size(), 2).end(FIRST_BLOCK, 10);
