    ...
    public void sortPoliciesBySubjectHieraichy() {
        ...
        for (Map.Entry<String, Assertion> entry : model.get("p").entrySet()) {
            ...
            Collections.sort(assertion.policy, (o1, o2)->{
                // BLOCKTEST EVAL: https://github.com/casbin/jcasbin/blob/eb7f93118e4f6d93e44cd779ce17a2f5f2d81387/src/main/java/org/casbin/jcasbin/model/Model.java#L331-L337
                blocktest().given(subjectHierarchyMap, new HashMap<String, Integer>(){{put("foo", 5); put("bar", 10);}}).given(o1, new ArrayList<String>(Arrays.asList("1", "2")), "List<String>").given(o2, new ArrayList<String>(Arrays.asList("1", "2")), "List<String>").given(domainIndex, -1).given(defaultDomain, "foo").mock("getNameWithDomain(domain1, o1.get(0))", "foo", "String").given("getNameWithDomain(domain2, o2.get(0))", "bar", "String").checkReturnEq(5);
