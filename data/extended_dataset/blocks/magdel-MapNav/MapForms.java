               ...
               // listIMapsAction();//GEN-LINE:|7-commandAction|138|597-postAction
                ...
                if ((id>0)&&(id<listIMaps.size()-1)){
                    ...
                    for (int i=0; i<RMSOption.IMAPS_NAMES.length; i++) {
                        // BLOCKTEST EVAL: https://github.com/magdel/MapNav/blob/97c4f93f02a56aa1ce2b2ffc8d0cd41ed150e9b3/src/main/java/app/MapForms.java#L1022C1-L1049C22
                        blocktest().given(i, 0).given(id, 1).mock("RecordStore.deleteRecordStore(..)").given(mn, new String[5]).given(mt, new String[5]).given(mp, new String[5]).given(mc, new float[5])
                                .given(RMSOption.IMAPS_NAMES, new String[]{"1", "2", "3", "4", "5"}, "String[]").given(RMSOption.IMAPS_RMST, new String[]{"6", "7", "8", "9", "10"}, "String[]")
                                .given(RMSOption.IMAPS_RMSP, new String[]{"a", "b", "c", "d", "e"}, "String[]").given(RMSOption.IMAPS_CENT, new float[]{1.2f, 3.4f, 5.6f, 7.8f, 9.10f}, "float[]")
                                .checkEq(mn[0], "1").checkTrue(mn[1] == null).checkEq(mt[0], "6").checkTrue(mt[1] == null).checkEq(mp[0], "a").checkTrue(mp[1] == null).checkTrue(mc[0] == 1.2f).checkTrue(mc[1] == 3.4f).checkTrue(mc[2] == 5.6f).checkTrue(mc[3] == 0f);
                        blocktest().given(i, 1).given(id, 1).mock("RecordStore.deleteRecordStore(..)").given(mn, new String[5]).given(mt, new String[5]).given(mp, new String[5]).given(mc, new float[5])
                                .given(RMSOption.IMAPS_NAMES, new String[]{"1", "2", "3", "4", "5"}, "String[]").given(RMSOption.IMAPS_RMST, new String[]{"6", "7", "8", "9", "10"}, "String[]")
                                .given(RMSOption.IMAPS_RMSP, new String[]{"a", "b", "c", "d", "e"}, "String[]").given(RMSOption.IMAPS_CENT, new float[]{1.2f, 3.4f, 5.6f, 7.8f, 9.10f}, "float[]")
                                .checkEq(mn[0], null).checkTrue(mn[1] == null).checkEq(mt[0], null).checkTrue(mt[1] == null).checkEq(mp[0], null).checkTrue(mp[1] == null).checkTrue(mc[0] == 0f).checkTrue(mc[1] == 0f).checkTrue(mc[2] == 0f).checkTrue(mc[3] == 0f);
                        if (i!=id){
                            if (i<id){
