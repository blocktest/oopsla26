          ...
          if (gpsBinded) {
            ...
            if (!rotateMap){
             ...
             //float pp=((dmaxx<dmaxy)?dmaxx:dmaxy)*sc/7;
              ...
              blocktest().given(dmaxx, 1, "int").given(dmaxy, 2, "int").given(GPSReader.SPEED_KMH, 40, "int").given(GPSReader.COURSE, 20, "int").given(MapUtil.G2R, 10, "int").checkEq(dx, 0).checkEq(dy, 0);
              blocktest().given(dmaxx, 200, "int").given(dmaxy, 100, "int").given(GPSReader.SPEED_KMH, 300, "int").given(GPSReader.COURSE, 50, "int").given(MapUtil.G2R, 2, "int").checkEq(dx, 14).checkEq(dy, -24);
             float pp=((dmaxx<dmaxy)?dmaxx:dmaxy)*((GPSReader.SPEED_KMH<40)?((GPSReader.SPEED_KMH>10)?(GPSReader.SPEED_KMH-10)/15:0):2)/7; 
