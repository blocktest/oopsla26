  ...
  public final LinkerDef getLinker(final AbstractCompileMojo mojo, final CCTask task, final String os, final String prefix,
    ...
    if ((this.narDependencyLibOrder == null) && (narDefaultDependencyLibOrder)) {
         if (os.equals(OS.AIX) && (getName(null, null).equals("xlC_r") || getName(null, null).equals("xlC") || getName(null, null).equals("xlc"))){
            ...
            // BLOCKTEST EVAL: https://github.com/maven-nar/nar-maven-plugin/blob/e747b5ee080b243e0b817f722a43c9c203bb4df8/src/main/java/com/github/maven_nar/Linker.java#L408C1-L417C14
            // Empty case
            blocktest().given(dependencies, "").given(dependency_list, new ArrayList<>())
                    .checkEq(libOrder.toString(), "").start(FIRST_BLOCK, 2);
            // Non-empty case
            blocktest().given(dependencies, "a, b,c , d ,e")
                    .given(dependency_list, new ArrayList<>(Arrays.asList("a", "b", "c", "d", "e")))
                    .checkEq(libOrder.toString(), "e,d,c,b,a").start(FIRST_BLOCK, 2);
    ...
    if (linkPaths != null && linkPaths.size() > 0 && mojo.getDirectDepsOnly() && !type.equals(Library.STATIC) && (os.equals(OS.LINUX) || os.equals(OS.AIX))){
        ...
        // BLOCKTEST EVAL: https://github.com/maven-nar/nar-maven-plugin/blob/e747b5ee080b243e0b817f722a43c9c203bb4df8/src/main/java/com/github/maven_nar/Linker.java#L430C1-L442C50
        // Case Linux with non-empty path
        blocktest().given(os, OS.LINUX).given(linkPaths, new ArrayList<>(Arrays.asList("foo", "bar")))
                .start(FIRST_BLOCK, 4).checkEq(argStr, "-Wl,-rpath-link,foo:bar:");
        // Case Linux with empty path
        blocktest().given(os, OS.LINUX).given(linkPaths, new ArrayList<>())
                .start(FIRST_BLOCK, 4).checkEq(argStr, "-Wl,-rpath-link,");
        // Case AIX with non-empty path
        blocktest().given(os, OS.AIX).given(linkPaths, new ArrayList<>(Arrays.asList("foo", "bar")))
                .start(FIRST_BLOCK, 4).checkEq(argStr, "-Lfoo:bar:");
        // Case AIX with empty path
        blocktest().given(os, OS.AIX).given(linkPaths, new ArrayList<>())
                .start(FIRST_BLOCK, 4).checkEq(argStr, "-L");
        // Case other systems with empty path
        blocktest().given(os, OS.MACOSX).given(linkPaths, new ArrayList<>(Arrays.asList("foo", "bar")))
                .start(FIRST_BLOCK, 4).checkEq(argStr, "foo:bar:");
        // Case other systems with non-empty path
        blocktest().given(os, OS.MACOSX).given(linkPaths, new ArrayList<>())
                .start(FIRST_BLOCK, 4).checkEq(argStr, "");
