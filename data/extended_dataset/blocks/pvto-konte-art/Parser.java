...
public class Parser {
    ...
    public static int getExpressionList(ArrayList<Tokenizer.TokenizerString> ttOrig, int startpos, ArrayList<Token> ret, int isSemicolons) throws ParseException {
        ...
        if (isSemicolons != 1) {
            if (t == Language.left_bracket || t instanceof Function || t instanceof InnerExpressiveToken) {
                ...
                if (u == null || u == Language.left_bracket || u instanceof Function || (u != null && InnerExpressiveToken.class.isAssignableFrom(u.getClass()))) {
                    // skip to loop
                } else {
        ...
        while (moveToNext) {
            ...
            last = ii > startpos ? t : null;
            t = Language.tokenByName(s);
            if (t == null) {
                t = new Token(s);
            }
            if (t == Language.right_curl || t == Language.semicolon || (lbcount == 0 && t == Language.comma)) {
                moveToNext = false;
                break;
                ...
                if (startpos < ii && !(last == Language.left_bracket || last instanceof Operator || last instanceof Comparator || last == Language.comma || Function.class.isAssignableFrom(last.getClass()))) {
                    moveToNext = false;
                    break;
                }
                lbcount++;
                ...
                if (lbcount <= 0 && isSemicolons != 1) {
                    moveToNext = false;
                    if (isSemicolons == 0 && ttOrig.size() > ii + 1 && Language.tokenByName(ttOrig.get(ii + 1).getString()) instanceof Operator) {
                        moveToNext = true;
                    }
                }
            } else if (startpos < ii && (last.getClass() == Constant.class || last.getClass() == Token.class || InnerExpressiveToken.class.isAssignableFrom(last.getClass())) && !(t instanceof Operator || t instanceof Comparator || lbctmp > 0 && t == Language.comma)) {
                moveToNext = false;
                break;
            }
            if (moveToNext && ii > startpos && (last == null || (lbctmp == 0 && last == Language.right_bracket)) && !(t instanceof org.konte.lang.Tokens.Operator || t instanceof Comparator || t == Language.right_curl || t == Language.right_bracket || t == Language.comma || t == Language.semicolon)) {
                moveToNext = false;
                break;
            }
            ret.add(t);
        ...
        blocktest().mock("ret.stream().map(tok -> tok.name).reduce(\"\", (ss, s) -> ss + \" \" + s)", "").given(t, Language.right_bracket).given(last, null).given(ret, new ArrayList<>(Arrays.asList(new Token("foo"), new Token("bar"), new Token("baz")))).given(lbcount, 1).given(lineNr, 1, "int").given(caretPos, 1, "int").end(FIRST_THROW).expect(ParseException.class).checkFlow(IfStmt().Then());
        blocktest().mock("ret.stream().map(tok -> tok.name).reduce(\"\", (ss, s) -> ss + \" \" + s)", "").given(t, Language.right_bracket).given(last, null).given(ret, new ArrayList<>(Arrays.asList(new Token("foo"), new Token("bar"), new Token("baz")))).given(lbcount, -1).given(lineNr, 1, "int").given(caretPos, 1, "int").end(FIRST_THROW).expect(ParseException.class).checkFlow(IfStmt().ElseIf().Then());
        blocktest().mock("ret.stream().map(tok -> tok.name).reduce(\"\", (ss, s) -> ss + \" \" + s)", "").given(t, Language.right_bracket).given(last, null).given(ret, new ArrayList<>(Arrays.asList(new Token("foo"), new Token("bar"), new Token("baz")))).given(lbcount, 0).given(lineNr, 0, "int").given(caretPos, 1, "int").end(FIRST_THROW).checkFlow(IfStmt().ElseIf().Else().Then().IfStmt());
        blocktest().mock("ret.stream().map(tok -> tok.name).reduce(\"\", (ss, s) -> ss + \" \" + s)", "").given(t, Language.right_bracket).given(last, new org.konte.lang.Tokens.Operator("+")).given(ret, new ArrayList<>(Arrays.asList(new Token("foo"), new Token("bar"), new Token("baz")))).given(lbcount, 0).given(lineNr, 0, "int").given(caretPos, 1, "int").end(FIRST_THROW).checkFlow(IfStmt().ElseIf().Else().Then().IfStmt()).expect(ParseException.class);
        if (lbcount > 0) {
            throw new ParseException("Missing  )  after\n" + ret.stream().map(tok -> tok.name).reduce("", (ss, s) -> ss + " " + s) + "\n", lineNr, caretPos);
        } else if (lbcount < 0) {
            throw new ParseException("Unexpected  ) \n " + ret.stream().map(tok -> tok.name).reduce("", (ss, s) -> ss + " " + s) + "\n", lineNr, caretPos);
        } else {
            if (t == Language.right_bracket && last instanceof org.konte.lang.Tokens.Operator)
                throw new ParseException("Expression terminates with " + last, lineNr, caretPos);
        }
        Runtime.sysoutln(ret, -1);
        return ii - 1;
    }

    // line number in source for parse exceptions
    static int lineNr;

    // caret position (running) in source, for parse exceptions
    static int caretPos;

    static final Pattern prefixPattern = Pattern.compile("^\\^(\\w+\\\\?)\\^(.*)");
    ...
    public Model parse(ArrayList<Tokenizer.TokenizerString> tokenStrings) throws ParseException {
        ...
        Model m = new Model();
        Name.model = m;
        // clear image reference hash from earlier parsings
        m.bitmapCache.clearReferences();
        ArrayDeque<ParsingContext> contextStack = new ArrayDeque<ParsingContext>();
        ParsingContext curCtx = ParsingContext.GRAMMAR;
        // last transform (inside rule transform)
        Transform ltfm = null;
        // last repeat transform (inside repeat)
        Transform lrstfm = null;
        ...
        for (int i = 0; i < tokenStrings.size(); i++) {
            ...
            try {
                switch(curCtx) {
                    case GRAMMAR:
                        ...
                        if (t == Language.rule) {
                            ...
                            if (t != null)
                                throw new ParseException("Misplaced token at root level: " + s, lineNr, caretPos);
                            curCtx = ParsingContext.RULE_CREATE;
                            i--;
                        }
                        break;
                    case MODEL_CREATE:
                        if (t == Language.left_curl) {
                            ...
                            throw new ParseException("Misplaced token after 'model': " + s, lineNr, caretPos);
                        }
                        break;
                    case MODEL:
                        if ((pos & 8) != 0) {
                            ...
                            throw new ParseException("model - not expecting " + s + "", lineNr, caretPos);
                        }
                        break;
                    case BACKGROUND_CREATE:
                        if (t == Language.left_curl) {
                            ...
                            throw new ParseException("Misplaced token after 'background': " + s, lineNr, caretPos);
                        }
                        break;
                    case BACKGROUND:
                        if (t instanceof InnerToken) {
                            if ((t == Language.R) || (t == Language.G) || (t == Language.B) || (t == Language.A) || (t == Language.H) || (t == Language.S) || (t == Language.L) || (t == Language.RGB)) {
                                lastInnerToken = t;
                                isSpecialContext = true;
                                lexprs.clear();
                                pos = 0;
                            } else {
                                throw new ParseException("background - not expecting " + s + ". Valid tokens are R G B A H S L RGB", lineNr, caretPos);
                            }
                        } else if (t == Language.right_curl) {
                            lastInnerToken = null;
                            curCtx = contextStack.pop();
                    ...
                    case INCLUDE:
                        ...
                        blocktest().given(s, "baz").given(t, new Token("foo")).given(lastName, "bar").given(lineNr, 0, "int").given(caretPos, 0, "int").expect(ParseException.class).checkFlow(IfStmt().Then().IfStmt().Then()).end(FIRST_BLOCK, 2);
                        blocktest().given(s, "baz").given(t, new Token("foo")).given(lastName, null).given(lineNr, 0, "int").given(caretPos, 0, "int").expect(ParseException.class).checkFlow(IfStmt().Then().IfStmt().Else()).end(FIRST_BLOCK, 2);
                        blocktest().given(s, "baz").given(t, null).given(lastName, "bar").given(lineNr, 0, "int").given(caretPos, 0, "int").checkEq(lastName, "bar baz").end(FIRST_BLOCK, 2);
                        blocktest().given(s, "baz").given(t, null).given(lastName, null).given(lineNr, 0, "int").given(caretPos, 0, "int").checkEq(lastName, "baz").end(FIRST_BLOCK, 2);
                        // .checkFlow(IfStmt().IfStmt().Then().IfStmt().Else()).end(FIRST_BLOCK, 2);
                        blocktest().given(s, "bar.png").given(t, null).given(lastName, null).given(lineNr, 0, "int").given(caretPos, 0, "int").checkEq(lastName, "bar.png").end(FIRST_BLOCK, 2);
                        if (t != null) {
                            if (lastName != null) {
                        ...
                        Matcher imgMatcher = Pattern.compile("(.*(jpg|jpeg|png|gif|JPG|PNG|GIF))\\s*(\\w*)$").matcher(lastName);
                        Matcher csvMatcher = Pattern.compile("(.*(csv|tsv|CSV|TSV|data))\\s*(\\w*)$").matcher(lastName);
                        Matcher nilMatcher = Pattern.compile("(/dev/null/?|null|nil|empty)\\s*(\\w*)$").matcher(lastName);
                        if (imgMatcher.find()) {
                            try {
                                ...
                                throw new ParseException(String.format("bitmap %s not loaded: %s", imgMatcher.group(1), ex.getMessage()), lineNr, caretPos);
                            }
                            ...
                            try {
                                ...
                                Runtime.sysoutln(String.format("%s [%d lines x %d columns] included as %s", fl.getAbsolutePath(), dataTable.data.size(), dataTable.headers.length, refName), 5);
                                ...
                                ex.printStackTrace();
                                throw new ParseException(String.format("csv/tsv %s not loaded: %s", csvMatcher.group(1), ex.getMessage()), lineNr, caretPos);
                            }
                            ...
                            if (tmp != null && tmp.length() > 0) {
                                ArrayList<Tokenizer.TokenizerString> included = Tokenizer.retrieveTokenStrings(tmp.toString());
                    ...
                    case SYSTEM:
                        if (lastName == null) {
                            ...
                            if (gn == null)
                                throw new ParseException("Expecting 'system <type>', found " + lastName + " instead. (Types are: " + GreyBoxSystem.Names.values(), lineNr, caretPos);
                        ...
                        if (t == Language.init) {
                            ...
                            try {
                                ...
                                throw new ParseException(pex.getMessage(), lineNr, caretPos);
                            }
                            tmpexps.clear();
                            ExpressionFunction func = (ExpressionFunction) lexpr;
                            Object[] vals = new Object[func.getArgs().length + 1];
                            // insert model as last argument
                            vals[vals.length - 1] = m;
                            int it = 0;
                    ...
                    case LIGHT:
                        if (t == Language.right_curl) {
                            ...
                            if (!matched) {
                                throw new ParseException("Unknown light type: " + s + " – known types are " + Arrays.toString(LightModifier.values()), lineNr, caretPos);
                            }
                        }
                        break;
                    case SHADING_CREATE:
                    ...
                    case STARTSHAPE:
                        if (t != null) {
                            throw new ParseException("Misplaced token after startshape: " + s, lineNr, caretPos);
                        } else {
                            if (Language.returnAsValue(s) != null) {
                                throw new ParseException("Not expecting a numerical constant after startshape", lineNr, caretPos);
                            }
                            m.startshape = s;
                            curCtx = contextStack.pop();
                        }
                    ...
                    case CAMERA_CREATE:
                        if (t == Language.left_curl) {
                            ...
                            throw new ParseException("Misplaced token after camera: " + s, lineNr, caretPos);
                        }
                        break;
                    case CAMERA:
                        if (t instanceof InnerToken) {
                            if ((t == Language.x) || (t == Language.y) || (t == Language.z) || (t == Language.rx) || (t == Language.ry) || (t == Language.rz) || (t == Language.s)) {
                                lastInnerToken = t;
                            } else {
                                throw new ParseException("Not expecting " + s + " in camera declaration. Valid tokens are: x y z rx ry rz s", lineNr, caretPos);
                            }
                            ...
                            boolean used = false;
                            for (CameraProperties prop : CameraProperties.values()) if (s.equalsIgnoreCase(prop.toString())) {
                                if (lastInnerToken != null)
                    ...
                    case MACRO_NAME:
                        if (t == null) {
                            if (Language.isName(s)) {
                                ...
                                throw new ParseException("Missing MACRO/DEF name near " + s, lineNr, caretPos);
                            }
                        } else {
                            throw new ParseException("Invalid token after MACRO/DEF: " + s, lineNr, caretPos);
                        }
                        break;
                    case MACRO_VALUE:
                        i = getExpressionList(tokenStrings, i, exprL);
                        ...
                        Runtime.sysoutln((isDef ? "DEF: " + lastName + "=" + val : "MACRO: " + lastName + "=" + lexpr), 0);
                        lastName = null;
                        isDef = false;
                        curCtx = contextStack.pop();
                        break;
                    ...
                    case RULE_CREATE:
                        if (t == null) {
                            if (Language.isName(s)) {
                                if (!wasNamed) {
                                    ...
                                    if (m.constants.get(s) != null) {
                                        throw new ParseException("Rule/path name reserved to a constant: " + s, lineNr, caretPos);
                                    }
                                    wasNamed = true;
                                } else {
                                    throw new ParseException("Invalid syntax: 'rule' or 'path' name must be followed by a positive weight or '{' " + "(near " + s + ")", lineNr, caretPos);
                                }
                            } else {
                                if (!wasNamed) {
                            ...
                            throw new ParseException("Misplaced token after 'rule' or 'path': " + s, lineNr, caretPos);
                        }
                        break;
                    case PATH:
                        if (t == null && s.trim().toLowerCase().startsWith("m ")) {
                            ...
                            if (svgi.initDocument(new ByteArrayInputStream(path.toString().getBytes())) != null) {
                                ...
                                ArrayList<Tokenizer.TokenizerString> included = Tokenizer.retrieveTokenStrings(kontePath);
                                if (included.size() > 0) {
                    ...
                    case RULE:
                        if (t == null || t instanceof Untransformable || t == Language.peek || t == Language.pop) {
                            if (Language.isName(s)) {
                                if (lastRule instanceof PathRule)
                                    throw new ParseException(String.format("call ('%s') within 'path' is prohibited", s), lineNr, caretPos);
                            ...
                            if (pr.steps.size() > 2 && pr.steps.get(pr.steps.size() - 1).type == PathRule.BEND && pr.steps.get(pr.steps.size() - 2).type == PathRule.BEND)
                            ...
                            throw new ParseException(String.format("Keyword '%s' prohibited in 'rule' block", s), lineNr, caretPos);
                        } else if (t == Language.subtract || t == Language.left_bracket || t instanceof Function) {
                            ...
                            // special case of repeat : apply transforms, call contained then once
                            lexpr = new Value(-1f);
                            contextStack.push(curCtx);
                            curCtx = Parser.ParsingContext.REPEAT_CREATE;
                            isSpecialContext = true;
                            ...
                            // curl is required by next state
                            i--;
                        } else if (t == Language.right_curl) {
                            isSpecialContext = false;
                            // clear our trail
                            ...
                            throw new ParseException("Misplaced token within " + lastRule + ": " + s, lineNr, caretPos);
                        }
                        break;
                    ...
                    case REPEAT_ADJUSTMENTS:
                        if (t instanceof InnerToken || t == Language.def || t == Language.undef || t == Language.right_curl || t == Language.left_curl) {
                            if (lastInnerToken == null && t instanceof InnerToken) {
                            ...
                            } else if (lastInnerToken != null && ((InnerToken) lastInnerToken).nParamsAllowed(pos)) {
                                // number of set arguments is (pos+1)-1 = pos
                                if (pos > 0) {
                                    if (curCtx == ParsingContext.TRANSFORM_ADJUSTMENTS)
                                        ltfm.setShapeTransform(lastInnerToken, lexprs);
                                    else
                                ...
                                throw new ParseException("Wrong number of arguments to " + lastInnerToken + " or missing () in " + lastRule, lineNr, caretPos);
                            }
                            if (t == Language.def || t == Language.undef) {
                                contextStack.push(curCtx);
                                isSpecialContext = false;
                                // 0 denotes def
                                pos = (t == Language.undef ? 1 : 0);
                                ...
                                if (lastInnerToken != null) {
                                    throw new ParseException("Missing argument(s) to " + lastInnerToken + " in " + lastRule, lineNr, caretPos);
                                }
                            ...
                            if (lastInnerToken == null) {
                                if (t == Language.left_bracket && i > 0 && Language.tokenByName(tokenStrings.get(i - 1).getString()) == null)
                            ...
                            if (lastInnerToken == Language.RGB && s.startsWith("#")) {
                                ...
                                if (curCtx == ParsingContext.TRANSFORM_ADJUSTMENTS) {
                                    ltfm.setShapeTransform(lastInnerToken, lexprs);
                                    if (b8) {
                                        ltfm.setShapeTransform(Language.A, aexpr);
                                    }
                                } else {
                                    lrstfm.setShapeTransform(lastInnerToken, lexprs);
                                    if (b8) {
                                        lrstfm.setShapeTransform(Language.A, aexpr);
                                    }
                                }
                                lastInnerToken = null;
                                break;
                            ...
                            try {
                                ...
                                if (lastInnerToken == Language.push && exprL.size() == 1) {
                                    ...
                                    if (matcher2.find()) {
                                        ...
                                        blocktest().given(prefixType, "AAA").given(tmpToken, new Token("foo")).given(lineNr, 1, "int").given(caretPos, 1, "int").given(i, 0).given(tokenStrings, new ArrayList<>(Arrays.asList(new Tokenizer.TokenizerString("a", 1, 1)))).mock("matcher2.group(1)", "foo").expect(ParseException.class).given(toPrefix, "abc").given(strait, 0).checkEq(i, -1);
                                        blocktest().given(exprL, new ArrayList<>(Arrays.asList(new Token("hey")))).given(prefixType, "m").given(tmpToken, new Token("foo")).given(lineNr, 1, "int").given(caretPos, 1, "int").given(i, 0).given(tokenStrings, new ArrayList<>(Arrays.asList(new Tokenizer.TokenizerString("a", 1, 1)))).checkEq(i, -1).mock("matcher2.group(1)", "foo").checkEq(tokenStrings.get(0).toString(), "^foo^bc").checkEq(exprL.get(0).toString(), "ma").given(toPrefix, "abc").given(strait, 0);
                                        blocktest().given(exprL, new ArrayList<>(Arrays.asList(new Token("hey")))).given(prefixType, "m").given(tmpToken, new Token("foo")).given(lineNr, 1, "int").given(caretPos, 1, "int").given(i, 0).given(tokenStrings, new ArrayList<>(Arrays.asList(new Tokenizer.TokenizerString("a", 1, 1)))).checkEq(i, 0).mock("matcher2.group(1)", "foo").checkEq(tokenStrings.get(0).toString(), "a").checkEq(exprL.get(0).toString(), "mf").given(toPrefix, "f").given(strait, 0);
                                        blocktest().given(exprL, new ArrayList<>(Arrays.asList(new Token("hey")))).given(prefixType, "m").given(tmpToken, new Token("foo")).given(lineNr, 1, "int").given(caretPos, 1, "int").given(i, 0).given(tokenStrings, new ArrayList<>(Arrays.asList(new Tokenizer.TokenizerString("a", 1, 1)))).checkEq(i, -1).mock("matcher2.group(1)", "foo").checkEq(tokenStrings.get(0).toString(), "^foo^ab").checkEq(exprL.get(0).toString(), "mc").given(toPrefix, "abc").given(strait, 1);
                                        ...
                                        if (toPrefix.length() > 1) {
                                            ...
                                            tokenStrings.set(i, new TokenizerString('^' + matcher2.group(1) + '^' + toPrefix.substring(1 - strait, toPrefix.length() - strait), tts.getLineNr(), tts.getCaretPos()));
                                            i--;
                                        }
                                    }
                                }
                            ...
                            switch(pos) {
                                ...
                                default:
                                    ...
                                    lexprs.add(lexpr);
                                    if (!((InnerToken) lastInnerToken).higherParamCountAllowed(pos + 1) && ((InnerToken) lastInnerToken).nParamsAllowed(pos + 1)) {
                                        if (curCtx == ParsingContext.TRANSFORM_ADJUSTMENTS)
                                            ltfm.setShapeTransform(lastInnerToken, lexprs);
                                        else
                                            lrstfm.setShapeTransform(lastInnerToken, lexprs);
                    ...
                    case TRANSFORM_DEFS:
                        ...
                        if (wasNamed && wasSet && isSpecialContext) {
                            ...
                            if (!wasNamed) {
                                if (wasSet)
                                    throw new ParseException("Orphaned  =  near " + s, lineNr, caretPos);
                                lastName = s;
                                wasNamed = true;
                            } else
                                throw new ParseException("Missing = in DEFinition block near " + s, lineNr, caretPos);
                        } else {
                            throw new ParseException("Misplaced token within DEF block: " + s, lineNr, caretPos);
                        }
                        break;
                    ...
                    case RULE_POST_CREATE:
                        if (t == Language.left_curl) {
                            switch(curCtx) {
                                case RULE_PRE_CREATE:
                                    curCtx = Parser.ParsingContext.RULE_PRE;
                                    break;
                                case RULE_POST_CREATE:
                                    curCtx = Parser.ParsingContext.RULE_POST;
                                    break;
                            }
                            lastValue = null;
                        } else {
                            throw new ParseException("Misplaced token after pre or post condition: " + s, lineNr, caretPos);
                        }
                        break;
                    case RULE_PRE:
                    case RULE_POST:
                        if (t == Language.semicolon || t == Language.comma) {
                            ...
                            throw new ParseException("Misplaced comparator " + lexpr + " in " + curCtx + " near " + s, lineNr, caretPos);
                        } else if (t == Language.right_curl) {
                            curCtx = contextStack.pop();
                        } else {
                            i = getExpressionList(tokenStrings, i, exprL, Language.IS_SEMICOLON_SEPARATORS ? 1 : 0);
                            ...
                            switch(curCtx) {
                                case RULE_PRE:
                                    lastRule.addPre((BooleanExpression) lexpr);
                                    break;
                                case RULE_POST:
                                    lastRule.addPost((BooleanExpression) lexpr);
                                    break;
                            }
                        }
                        break;
                    case REPEAT_CREATE:
                        if (t == Language.left_curl) {
                            ...
                            // ready for transform iteration
                            lrepeat = r;
                            // will be loaded in repeat block
                            lrstfm = new Transform(lineNr, caretPos);
                            ...
                            throw new ParseException("Misplaced token within repeat block: " + s, lineNr, caretPos);
                        }
                        break;
                    case BLOCK_CREATE:
                    case IF_CREATE:
                        if (t == Language.left_bracket) {
                            if (isSpecialContext)
                                throw new ParseException("Expecting ')' - found '" + t + "' after conditional", lineNr, caretPos);
                            isSpecialContext = true;
                            ...
                            } else if (conditionalStack.size() > 1) {
                                Iterator it = conditionalStack.iterator();
                                it.next();
                                ((ConditionalStructure) it.next()).onCondition.add(cond);
                            } else
                                lastRule.addTransform(cond);
                            // repeats always inside if, not the other way round
                            lrepeat = null;
                            curCtx = Parser.ParsingContext.IF;
                        } else {
                            throw new ParseException("Misplaced token within block definition: " + s, lineNr, caretPos);
                        }
                        break;
                    case IF:
                        if (t == null || t instanceof Untransformable || t == Language.peek || t == Language.pop) {
                            ...
                            throw new ParseException(String.format("Keyword '%s' prohibited in 'if' block", s), lineNr, caretPos);
                        } else if (t == Language.subtract || t == Language.left_bracket || t instanceof Function) {
                            i = getExpressionList(tokenStrings, i, exprL);
                            lexpr = exprParser.parse(exprL, 0, m);
                            contextStack.push(curCtx);
                ...
                // new ParseException(ex.getMessage(),lineNr,caretPos);
                throw ex;
            } catch (Exception ex) {
                ex.printStackTrace();
                throw new ParseException(ex.getMessage(), lineNr, caretPos);
