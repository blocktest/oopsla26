    ...
    public Object invoke(RpcInvocation rpcInvocation) {
        ...
        if (CompletableFuture.class.isAssignableFrom(rpcInvocation.getMethod().getReturnType())) {
            ...
            defaultFuture.addCallback((result, exception) -> {
                ...
                blocktest().given(result, "foo", "Object").given(exception, new FileNotFoundException(), "Throwable").checkTrue(ret.isCompletedExceptionally());
                blocktest().given(result, "foo", "Object").given(exception, null, "Throwable").checkTrue(ret.isDone());
                if (exception != null) {
