    ...
    private static void setTermsAggregation(SearchSourceBuilder searchSource, TermsAggregation aggregation, Integer size) {
        ...
        if (aggregation.order().length > 0) {
            List<BucketOrder> orders = Stream.of(aggregation.order()).distinct().map(t -> {
                // BLOCKTEST EVAL: https://github.com/Thare-Lam/fast-elasticsearch-query-builder/blob/bc6e2adf8333363929e68f3c8dc459c320101aef/src/main/java/cn/thare/feqb/helper/AggregationHelper.java#L42-L55
                blocktest().given(t, TermsAggregation.Order.COUNT_ASC, "TermsAggregation.Order").checkReturnEq(BucketOrder.count(Boolean.TRUE));
                blocktest().given(t, TermsAggregation.Order.COUNT_DESC, "TermsAggregation.Order").checkReturnEq(BucketOrder.count(Boolean.FALSE));
                blocktest().given(t, TermsAggregation.Order.KEY_ASC, "TermsAggregation.Order").checkReturnEq(BucketOrder.key(Boolean.TRUE));
                blocktest().given(t, TermsAggregation.Order.KEY_DESC, "TermsAggregation.Order").checkReturnEq(BucketOrder.key(Boolean.FALSE));
                switch (t) {
                    case COUNT_ASC:
