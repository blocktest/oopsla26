    ...
    public static SQLDataType transformOracleToMySql(SQLDataType x) {
        ...
        if (nameHash == FnvHash.Constants.UROWID) {
            ...
            if (argumentns.size() > 0) {
                ...
                dataType = new SQLCharacterDataType("nvarchar", len);
                // BLOCKTEST EVAL: https://github.com/Isaac315/sql-translator/blob/78f3fb0091c94563a640ef672de3e75e2b5c3bb2/src/main/java/com/raysonfang/sqltranslator/sql/dialect/oracle/util/OracleSQLDataTypeTransformUtil.java#L209C1-L216C70
                // Case if
                blocktest().given(len, 0).given(x, new OracleDataTypeIntervalDay()).given(arg0, new SQLNumberExpr(12)).checkEq(dataType, new SQLCharacterDataType("nvarchar", 12)).start(FIRST_BLOCK);
                // Case else
                blocktest().given(len, 100).given(x, new OracleDataTypeIntervalDay()).given(arg0, new OracleArgumentExpr()).expect(UnsupportedOperationException.class).start(FIRST_BLOCK);
            } else {
