...
# Usage: bash run_blocktest_with_time.sh <block-url> <extension_dir> <output_dir> [duplicate=1]
#
...
check_inputs() {
	if [[ ! -f ${BLOCKTEST_FILE} || ! -d ${EXTENSION_DIR} || -z ${OUTPUT_DIR} ]]; then
		echo "Usage: bash run_blocktest_with_time.sh <block-url> <extension_dir> <output_dir> [duplicate=1]"
		exit 1
	fi
...
generate_test() {
	...
	if [[ -n ${DUPLICATE} ]]; then
		duplicated_test_count=" --duplicated_test_count=${DUPLICATE}"
	fi
...
run_blocktest() {
	...
	# Dry run
	if [[ -n ${DUPLICATE} ]]; then
		# Run a shorter empty test to download deps
		(time mvn ${SKIP} -Djava.io.tmpdir=${tmp_dir} -Dmaven.repo.local=${OUTPUT_DIR}/repo -Dmaven.ext.class.path=${extension_jar} test-compile surefire:test -Dtest="${TEST_FILENAME}#testEmptyForDuplicateExp") &> ${OUTPUT_DIR}/logs/run.log
		check_status_code "run_blocktest"
	else
		(time mvn ${SKIP} -Djava.io.tmpdir=${tmp_dir} -Dmaven.repo.local=${OUTPUT_DIR}/repo -Dmaven.ext.class.path=${extension_jar} test-compile surefire:test -Dtest="${TEST_FILENAME}") &> ${OUTPUT_DIR}/logs/run.log
		check_status_code "run_blocktest"
	fi
	...
	local start=$(date +%s%3N)
	(time mvn ${SKIP} -Djava.io.tmpdir=${tmp_dir} -Dmaven.repo.local=${OUTPUT_DIR}/repo -Dmaven.ext.class.path=${extension_jar} test-compile surefire:test -Dtest="${TEST_FILENAME}") &> ${OUTPUT_DIR}/logs/run-timed.log
	check_status_code "run_blocktest_timed"
	local end=$(date +%s%3N)
	DURATION=$((end - start))
	
	if [[ -d target/surefire-reports ]]; then
		cp -r target/surefire-reports ${OUTPUT_DIR}/reports
	fi
